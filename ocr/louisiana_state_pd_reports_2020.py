...
def process_all_pdfs() -> pd.DataFrame:
    dir_name = real_dir_path("raw_louisiana_state_pd_reports_2020.dvc")
    return (
        pd.read_csv(deba.data("meta/reports_louisiana_state_pd_2020_files.csv"))
        .pipe(only_pdf)
...
if __name__ == "__main__":
    df = process_all_pdfs()
    df.to_csv(deba.data("ocr/reports_louisiana_state_pd_2020_pdfs.csv"), index=False)
