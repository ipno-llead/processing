...
spec_chars = [r'Date/Time of Incident: ', r'hours', r'Case Number', r'hrs', "!",'"',"%","&","'","(",")",
              "*","+",",","-",".",":",";","<",
              "=",">","?","[","\\","]","^","_",
              "`","{","|","}","~","–", '\n', r'Date', r'Filed']
...
for char in spec_chars:
    df1['date_filed' ] = df1['date_filed'].str.replace(char, ' ').str.split().str.join(" ")
...
df5['disposition'] = df5['disposition'].map(lambda x: x.lstrip('5.').rstrip('\n'))\
    .str.split().str.join(" ").str.capitalize()
...
for char in spec_chars:
    df6['occur_date' ] = df6['occur_date'].str.replace(char, ' ').str.split().str.join(" ")
df6 = pd.DataFrame(df6['occur_date'].str.strip().str.split('@',1).tolist(),
...
for char in spec_chars:
    df7['case_number' ] = df7['case_number'].str.replace(char, ' ').str.split().str.join(" ")
...
df.to_csv (r'titport_allen_pd_cprr_2019_forms.csv', index=None)
