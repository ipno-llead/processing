...
def set_file_category(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df.loc[df.fn.astype(str).str.match(r"(.+)"), column] = "nopd_appeal_hearing_transcript"
    return df


def fetch_reports() -> pd.DataFrame:
    return (
        files_meta_frame("raw_nopd_appeals.dvc")
        .pipe(set_filetype)
        .pipe(split_filepath)
...
if __name__ == "__main__":
    df = fetch_reports()
    df.to_csv(deba.data("meta/nopd_appeals_files.csv"), index=False)
