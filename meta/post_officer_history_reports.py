...
def fetch_reports_9_30_2022_rotated() -> pd.DataFrame:
    return (
        files_meta_frame("raw_post_officer_history_reports_9_30_2022_rotated.dvc")
        .pipe(set_filetype)
        .pipe(split_filepath)
        .pipe(set_file_category)
    )
...
if __name__ == "__main__":
    ...
    df_930_22_rotated = fetch_reports_9_30_2022_rotated()
    df21.to_csv(deba.data("meta/post_officer_history_reports_files.csv"), index=False)
    df22.to_csv(
        deba.data("meta/post_officer_history_reports_9_16_2022_files.csv"), index=False
    )
    df_adv.to_csv(
        deba.data("meta/post_officer_history_reports_advocate_files.csv"), index=False
    )
    df23_rotated.to_csv(
        deba.data("meta/post_officer_history_reports_2023_rotated_files.csv"),
        index=False,
    )
    df_930_22_rotated.to_csv(
        deba.data("meta/post_officer_history_reports_9_30_2022_rotated_files.csv"),
        index=False,
    )
