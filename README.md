...
- **Python 3.9+**: We recommend installing Python 3.9 with [pyenv](https://github.com/pyenv/pyenv) and then create an env just for this project with [virtualenvwrapper](https://virtualenvwrapper.readthedocs.io/en/latest/).
...
This integration pipeline produce the following kind of data for each police agency:
...
1. **Add raw CSVs**: CSV files that serve as input for this entire pipeline are called "raw" as they are not yet cleaned, and need to be integrated with the rest of the data. Modify `raw_datasets.json` and add links to the raw CSVs under the name of the agency that those files belong to. As a convention, the name of the agency have to be in snake case. The links have to be public so that anyone can checkout this repo and download the raw files (note that for Dropbox links you need to add `?dl=1` suffix for them to work).
...
11. **Pull latest data**: Run `wrgl pull --all`. This pulls all latest data changes for all branches.
12. **Review your changes**: Run `wrgl diff --all` to see all the changes you made with all existing branches. Run `wrgl diff {branch}` to review detailed changes for a single branch.
13. **Commit new data**: When you are satisfied with the changes, commit new data with `wrgl commit --all "{commit message}"`. The commit message should be short and describe the changes that you made. Something similar to the Git commit message is good.
14. **Push new data**: Run `wrgl push --all`.

### 4.b. Extract tables from PDFs with Form Recognizer

This repository proposes a workflow and some utilty scripts to help extracting tables from PDF with [Azure From Recognizer](https://azure.microsoft.com/en-us/services/form-recognizer/). There are 2 workflows:
