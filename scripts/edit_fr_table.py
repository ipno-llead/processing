...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Edit FormRecognizer table labels.')
    parser.add_argument(
        'pdf_path', type=str, metavar='PDF_PATH', help='Path of the PDF whose labels need to be edit'
    )
    parser.add_argument(
        'table_tag', type=str, metavar='TABLE_TAG', help='Name of the table tag to edit'
    )
    parser.add_argument(
        'op', choices=['insertRow', 'removeRow'], metavar='OP', help='Operation to perform on the table'
    )
    parser.add_argument(
        'start', type=int, metavar='START', help='index of the first row to operate on'
    )
    parser.add_argument(
        'count', type=int, metavar='COUNT', default=1, help='the number of rows to insert/remove'
    )
    ...
    blob_client = blob_service_client.get_blob_client(container=container, blob=labels_path)
    ...
    for idx, obj in enumerate(labels['labels']):
        ...
        if args.op == 'insertRow':
            obj['label'] = '%s/%d/%s' % (args.table_tag, row_idx + args.count, m.group(2))
            ...
            if row_idx < args.start + args.count:
                ...
                obj['label'] = '%s/%d/%s' % (args.table_tag, row_idx - args.count, m.group(2))
    labels['labels'] = [
        obj for idx, obj in enumerate(labels['labels'])
        if idx not in labels_to_remove
    ]
