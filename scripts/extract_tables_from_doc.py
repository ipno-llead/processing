...
class PDFBatchReader(object):
    def __init__(self, build_dir, doc_url, batch_size=10, end_page=math.inf):
        ...
        self.pbar = tqdm(total=page_count, desc='analyzing PDF pages {batch_size=%d}' % (self.batch_size))
    ...
    def split_pdf_into_batches(self):
        with open(self.build_dir / 'pdf_batch_reader/docs' / self.filename, 'rb') as f:
            ...
            with open(self.batch_dir / 'pageCount', 'w') as cf:
                cf.write('%d' % total)
            print("splitting pdf (%d pages) into %d batches" % (total, int(math.ceil(float(total) / self.batch_size))))
...
class TableExtractor(object):
    ...
    def get_writer(self, name, fieldnames):
        if name not in self._writers:
            ...
            self._writers[name] = csv.DictWriter(self._file_handles[name], fieldnames=fieldnames)
            if not exists:
    ...
    def extract_tables_from_extracted_documents(self, batch_idx, result):
        for idx, doc in enumerate(result.documents):
            for field_name, field in doc.fields.items():
                if field.value_type != 'list' or field.value[0].value_type != 'dictionary':
                    print('field %s in document %d is not a table: %s' % (field_name, idx, field))
                    continue
                fieldnames = list(field.value[0].value.keys()) + ['batch_index', 'table_index']
...
if __name__ == '__main__':
    ...
    with TableExtractor(args.build_dir, args.csv_path, args.trim_selection_marks) as ex:
        reader = PDFBatchReader(args.build_dir, args.doc_url, args.batch_size, args.end_page)
