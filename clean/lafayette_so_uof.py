...
from lib.clean import strip_leading_comma, clean_dates, clean_races, clean_sexes, standardize_desc_cols


def clean_medical_treatment(df):
    df.loc[:, "medical_treatment"] = (df.medical_treatment
                                      .str.lower()
                                      .str.strip()
...
def clean_use_of_force_description(df):
    df.loc[:, "use_of_force_description"] = (df.type_of_force
                                            .str.lower()
                                            .str.strip()
...
def clean_uof_reason(df):
    df.loc[:, "use_of_force_reason"] = (df.incident_type
                                        .str.lower()
                                        .str.strip()
                                        .str.replace(r"^dist\. ?w ?\/ ?weapons$", "disturbance with weapons", regex=True)
...
def clean_disposition(df):
    df.loc[:, "disposition"] = (df.justified
                                .str.lower()
                                .str.strip()
...
def clean_uof():
    df = (pd.read_csv(deba.data("raw/lafayette_so/lafayette_so_uof_2015_2019.csv"))
                      .pipe(clean_column_names)
                      .pipe(strip_leading_comma)
                      .drop(columns=["injury_deputy_suspect"])
                      .rename(columns={"date": "occur_date", 
                                       "race": "citizen_race", 
                                       "gender": "citizen_sex", 
                                       "age": "citizen_age",})
                     ...
                     .pipe(gen_uid, ["use_of_force_reason","use_of_force_description", "disposition", 
                                     "location", "occur_year", "occur_month", "uid", "tracking_id"], "uof_uid")
                     .pipe(gen_uid, ["citizen_race", "citizen_sex", "citizen_age", 
                                    "use_of_force_reason", "use_of_force_reason", "tracking_id"], "citizen_uid")
...
if __name__ == "__main__":
    ...
    citizen_df.to_csv(deba.data("clean/uof_citizens_lafayette_so_2015_2019.csv"), index=False)
