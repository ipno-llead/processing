...
def extract_name(df):
    ...
    df2 = pd.concat(
        [
            ...
            df2.iloc[:, 1]
            .str.extract(r"^([^ ]+)(?: (.+))?$")
            .rename(columns={0: "first_name", 1: "middle_name"}),
        ],
        axis=1,
    )
    rows_with_suffix = df2.middle_name.fillna("").str.match(
    ...
    df2.loc[rows_with_suffix, "last_name"] = df2.loc[
        rows_with_suffix, "last_name"
    ].str.cat(df3.loc[rows_with_suffix, 1], sep=" ")
...
def clean():
    return (
        pd.read_csv(dirk.data("raw/caddo_parish_so/caddo_parish_so_pprr_2020.csv"))
        ...
        .pipe(
            gen_uid, ["agency", "first_name", "last_name", "middle_name", "employee_id"]
        )
    )
...
if __name__ == "__main__":
    ...
    df.to_csv(dirk.data("clean/pprr_caddo_parish_so_2020.csv"), index=False)
