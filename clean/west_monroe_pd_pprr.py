...
def split_names(df):
    names = df.employee_name.str.lower().str.strip()\
        .str.extract(r'^(\w+),\s+(\w+)$')
...
def change_hire_date_format(df):
    df.loc[:, 'hire_date'] = df.hire_date.str.strip()\
...
def clean():
    return pd.read_csv(data_file_path(
        'raw/west_monroe_pd/west_monroe_pd_pprr_2015_2020.csv'
        ...
        .pipe(set_values, {
            'agency': 'West Monroe PD',
            'salary_freq': salary.MONTHLY
        })\
...
if __name__ == '__main__':
    ...
    df.to_csv(data_file_path(
        'clean/pprr_west_monroe_pd_2015_2020.csv'
    ), index=False)
