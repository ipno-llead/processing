...
def realign_18():
    ...
    df.loc[:, "Status"] = df["Status"].str.replace("-", "").str.strip().str.title()
    df.loc[:, "Received"] = df["Received"].str.replace("-", "").str.strip()
    df.loc[:, "Occur Date"] = (
        df["Occur Date"].str.replace("-", "").str.strip().fillna("")
    )
    df.loc[:, "Officer Name"] = (
        df["Officer Name"]
        .str.strip()
        ...
        .str.replace("none entered", "", regex=False)
        .fillna("")
    )
    df.loc[:, "Complaint"] = (
        df.loc[:, "Complaint"]
        ...
        .str.replace(r" \.(\d+)", r" \1")
        .str.replace(r" of$", "")
        .str.replace("Use Force", "Use of Force", regex=False)
        .str.replace(r" ?- ", " ")
        .str.replace(" lo ", " to ", regex=False)
        .str.replace("Emply", "Empty", regex=False)
        .str.replace("Oul ", "Out ", regex=False)
        .fillna("")
    )
    df.loc[:, "Action"] = (
        df["Action"]
        ...
        .str.replace("Sgl. ", "Sgt. ", regex=False)
        .str.strip()
        .fillna("")
    )
    df.loc[:, "Disposition"] = (
        ...
        .str.replace("Sustaned", "Sustained", regex=False)
        .str.strip()
        .fillna("")
    )
    df.loc[:, "Action"] = df["Action"].str.replace("Slops", "Stops", regex=False)
...
def parse_complaint_18(df):
    ...
    df.loc[
        (df.rule_code == "3:17") & (df.paragraph_code == "40"), "rule_paragraph"
    ] = "Carrying Out Orders / General Orders (Pursuit)"
...
def standardize_action_18(df):
    df.loc[:, "action"] = df.action.str.replace(
        r"(\d) day suspension", r"\1-day suspension"
...
def combine_rule_and_paragraph(df):
    ...
    df = df.drop(
        columns=["rule_code", "rule_violation", "paragraph_code", "paragraph_violation"]
    )
    return df
...
def clean_receive_and_occur_dates_21(df):
    df.loc[:, "receive_date"] = df.received.str.replace("- ", "", regex=False)
    df.loc[:, "occur_date"] = (
        df.occur_date.str.replace(r"^0/.+", "", regex=True)
...
def clean_complainant_21(df):
    df.loc[:, "complainant"] = (
        ...
        .str.replace(
            "brpdtim browning",
            "baton rouge police department/tim browning",
            regex=False,
        )
    )
    return df
...
def clean_allegations_21(df):
    df.loc[:, "allegation"] = (
        ...
        .str.replace(
            r"(failure to report)? ?(2:[87])? ?damag(ed?|ing)? ?(to)? "
        ...
        .str.replace(
            r"^carr[ry]?(ing)? out orders\b", "3:17 carrying out orders", regex=True
        )
        .str.replace(r"\((veh)? ?(pursuit)?\)$", "(pursuit) - 40", regex=True)
        .str.replace(
            r"(^22?:?[12]?2?)? ?command ?(of)? temper ?(13)?",
            "2:2 command of temper - 13",
            regex=True,
        )
        .str.replace(
            r"^(viol)?(ation)? ?of (known)? ?laws",
        ...
        .str.replace(
            r"(1:7)? ?(fail)?(ure)? ?(to)? ?(comp(lete?)?(ion)?)? "
        ...
        .str.replace(
            r"(2:5)? ?awol ?(15)?", "2:5 absent without leave - 15", regex=True
        )
        ...
        .str.replace(
            r"^(release of prisoner)? ?/? ?allow(ing)? ?escape ?(30)?",
            "3:7 release of prisoners / allowing escape - 30",
            regex=True,
        )
        .str.replace(
            r"(2:1[23])? ?respect ?(of)? ?(fellow)? ?(officers|members)? " r"?(22)?",
            "2:13 respect of fellow officers - 22",
            regex=True,
        )
        .str.replace(
            r"(3:9)? ?fail(ure)? ?(to)? ?(provide)? "
        ...
        .str.replace(
            r"^assc with known criminals$",
            "3:21 association with known criminals - 55",
            regex=True,
        )
        .str.replace(
            r"\(contact person\)45 failure to$", "(contact person) - 45", regex=True
        )
        ...
        .str.replace(
            "failure to report lost / 2:7 damaging department equipment - 18",
        ...
        .str.replace(
            r"^fail notify superv$", "failure to notify supervisor", regex=True
        )
        .str.replace(
            r"(2:?1[12])? ?(conduct)? ?unbecoming ?(an)? ?(officer)? ?-? ?(21)? ?(violation)?",
            "2:12 conduct unbecoming an officer - 21",
            regex=True,
        )
        .str.replace(r"(\w+) - (\d+) (\(\w+\))", r"\1 \3 - \2", regex=True)
        .str.replace(
            r"^abuse of sick leave$", "2:3 abuse of sick leave - 66", regex=True
        )
...
def parse_officer_name_21(df):
    ...
    return (
        df.dropna(subset=["name"]).reset_index(drop=True).drop(columns="officer_name")
    )
...
def split_name_21(df):
    df.name = (
        ...
        .str.replace(".", "", regex=False)
        .str.replace(
            r"(\w+) (\b\w{1}) (\b\w{2}\b) (?:(p\d+) )?", r"\2 \1 \3 \4 ", regex=True
        )
        ...
        .str.replace(
            r"^(\w+) (\w{2}) (\w+) (p\d+) (\w{1})$", r"\1 \5 \3 \2 \4", regex=True
        )
    ...
    names = (
        df.name.str.lower()
        .str.strip()
        .str.extract(r"(?:(lt|cpl|capt|ofc|sgt|maj|det) )?(?:([^ ]+) )?(\w+ )?(.+)")
    )
...
def split_department_and_division_desc_21(df):
    df.loc[:, "department_desc"] = (
        ...
        .str.replace(
            r"((operation service|communications)? comm center)$",
            "operations service communications center",
            regex=True,
        )
        .str.replace(r"\bcib\b", "criminal investigations", regex=True)
        .str.replace(
            r"^special$|^special operations tru$", "special operations", regex=True
        )
...
def clean_action_21(df):
    df.loc[:, "action"] = (
        ...
        .str.replace(r"(\d+)-? (\w+)", r"\1-\2", regex=True)
        .str.replace(
            r" ?(8)?(-)?hr\.? ?(dr)?i?v?\.?(iv?ng)? ?(sch)?(ool)? ?",
            "8-hour driving school",
            regex=True,
        )
        .str.replace(",", "/", regex=False)
        .str.replace(".", "", regex=False)
        ...
        .str.replace(
            r"mandatory training -? ?(accident)? ?investigation/de escalation",
            "mandatory accident and de-escalation training",
            regex=True,
        )
        .str.replace(
            "advanced training from firearms supervisor",
        ...
        .str.replace(
            r"/?crawford|/?iverson|/?srantz|/?hernandez|"
            r"/?thomas|/?ofc|/?cowardice|/?adkins",
            "",
            regex=True,
        )
        .str.replace(
            r"/?(unbecoming/?|/?officer?|/? ?investig?ation/? ?"
            r"|/? ?coo/?| from rso)",
            "",
            regex=True,
        )
        .str.replace(
            r"/? ?shirking ?|/? ?truth(ful?)? ?|hord/?|"
            r"blust/?|/?insub/?|/?respect/?|/?sexual harrass",
            "",
            regex=True,
        )
        .str.replace(
            r"/?consent|/?handled by lt/? dabadie|/?discipline"
            r"|/?carrying out orders|/?use of force/?",
            "",
            regex=True,
        )
        ...
        .str.replace(
            r"10-da suspensionletter of reprimandletterof caution",
            "10-day suspension/letter of reprimand/letter of caution",
            regex=False,
        )
        .str.replace(
            r"letter of reprimand8-hour driving school",
            "letter of reprimand/8-hour driving school",
            regex=False,
        )
...
def clean_receive_and_occur_date(df):
    ...
    df.loc[:, "occur_date"] = (
        df.occur_date.str.replace(r"^(\.|\-)$", "", regex=True)
    ...
    df = (
        ...
        .pipe(
            ...
            [
                ...
                ["2:1 use of alcohol: drugs - 12", "2:1 use of alcohol / drugs - 12"],
                ...
                ["3:19 falsification of documents - 44"],
                ["3:20 use of alcohol: drugs", "3:20 use of alcohol / drugs"],
                ["3:20 use of force"],
