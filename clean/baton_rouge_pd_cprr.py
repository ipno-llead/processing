...
def realign_18():
    df = pd.read_csv(
        dirk.data("raw/baton_rouge_pd/baton_rouge_pd_cprr_2018.csv"),
        encoding="latin1",
    )
...
def clean_21():
    df = pd.read_csv(dirk.data("raw/baton_rouge_pd/baton_rouge_pd_cprr_2021.csv")).pipe(
        clean_column_names
    )
    df = (
...
if __name__ == "__main__":
    df18 = clean_18()
    df21 = clean_21()
    df18.to_csv(dirk.data("clean/cprr_baton_rouge_pd_2018.csv"), index=False)
    df21.to_csv(dirk.data("clean/cprr_baton_rouge_pd_2021.csv"), index=False)
