...
def clean_reason_at_booking(df):
    df.loc[:, 'reason_at_booking'] = df.reason_at_booking.str.lower().str.strip()
    return df
...
def clean_charge_description(df):
    df.loc[:, 'charge_description'] = df.charge_description.str.lower().str.strip()\
