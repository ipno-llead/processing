...
def clean_sentence_date(df):
    df.loc[:, "sentence_date"] = (
        df.sentence_date.str.lower()
...
def clean_check_in_date(df):
    df.loc[:, "check_in_date"] = df.check_in_date.str.replace(
        r"(\d+)/(\d+)/(\d+)", r"\2/\3/\1", regex=True
    )
    return df
...
def clean_court_time(df):
    df.loc[:, "am_pm"] = (
        df.court_time_flag_am_pm.str.lower()
        .fillna("")
    ...
    df.loc[:, "court_time"] = df.court_time.str.cat(df.am_pm).str.replace(
        r"^(\w+)$", "", regex=True
    )
...
def clean_release_reason(df):
    df.loc[:, "release_reason"] = (
        df.release_reason.str.lower()
        .str.strip()
...
def clean_charge_description(df):
    df.loc[:, "charges"] = (
        df.charge_description.str.lower()
        .str.strip()
...
def clean_and_join_sentence_types(df):
    df.loc[:, "sentence_other"] = (
        df.sentence_other.str.lower()
        .str.strip()
        .fillna("")
        .str.replace(r"rel$", "release", regex=True)
    ...
    df.loc[:, "sentence_type"] = (
        df.sentence_other
        + ""
        + df.sentence_type.fillna("")
        .astype(str)
        .str.replace(r"\.", "", regex=True)
        .str.replace(r"(\d+)", "", regex=True)
    )
...
def clean_arresting_department(df):
    df.loc[:, "department_desc"] = (
        df.arrest_credit_txt.str.lower()
        .str.strip()
        .fillna("")
        .str.replace(r"\.", "", regex=True)
        ...
        .str.replace(
            r"^environmental rangers = dept of sanitatn$", "sanitation", regex=True
        )
        ...
        .str.replace(
            "scid  crime lab", "scientific criminal investigations", regex=False
        )
