...
def clean_tracking_number(df):
    df.loc[:, 'tracking_number'] = df.file\
        .str.replace(r'-$', '', regex=True)
    return df.drop(columns='file')


def clean_incident_dates(df):
    df.loc[:, 'incident_date'] = df.date_of_incident.str.lower().str.strip()\
...
def clean_allegation(df):
    df.loc[:, 'allegation_desc'] = df.allegation.str.lower().str.strip()\
...
def split_rows_with_multiple_dispositions(df):
    df.loc[:, 'disposition'] = df.outcome.str.lower().str.strip()\
...
def extract_allegation_from_disposition(df):
    df.loc[:, 'allegation'] = df.disposition.str.lower().str.strip()\
...
def clean_action(df):
    df.loc[:, 'action'] = df.disciplinary_outcome.str.lower().str.strip()\
        ...
        .str.replace(r'loss of (\w+)\.(\w+) hours ?(of)? ?pay', r'\1.\2-hours loss of pay', regex=True)\
        .str.replace(r'loss of (\w+) hours ?(of)? ?pay', r'\1-hours loss of pay', regex=True)\
        .str.replace(r'loss of (\w+) days? ?(of)? ?seniority', r'\1-day loss of seniority', regex=True)\
        .str.replace('loss of 1-day seniority', '1-day loss of seniority', regex=False)
    return df.drop(columns='disciplinary_outcome')


def clean_names(df):
    df.loc[:, 'officer_name'] = df.officer_name.fillna('').str.lower().str.strip()\
        .str.replace(r'sgt\.?', 'sergeant', regex=True)\
        .str.replace(r'lt\.?', 'lieutenant', regex=True)

    names = df.officer_name.str.extract(r'(officer|lieutenant|sergeant|reserve officer) (\w+)\.? (\w+)')
...
def clean():
    df = pd.read_csv(data_file_path('raw/denham_springs_pd/denham_springs_pd_cprr_2016_2021.csv'))\
        ...
        .pipe(set_values, {
            'agency': 'Denham Springs PD'
        })\
        .pipe(gen_uid, ['agency', 'first_name', 'last_name'])\
        .pipe(gen_uid, ['uid', 'allegation', 'allegation_desc', 'disposition', 'action'], 'allegation_uid')
    return df


if __name__ == '__main__':
    df = clean()
    df.to_csv(data_file_path('clean/cprr_denham_springs_pd_2016_2021.csv'), index=False)
