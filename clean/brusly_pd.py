...
def clean_cprr():
    df = pd.read_csv(deba.data("raw/brusly_pd/brusly_pd_cprr_2020.csv"))
...
def clean_position(df):
    df.loc[:, "rank_desc"] = (
        df.position.str.lower().str.strip().str.replace("police ", "", regex=False)
    )
...
def clean_pprr():
    df = (
        pd.read_csv(deba.data("raw/brusly_pd/brusly_pd_pprr_2020.csv"))
...
def clean_award():
    return (
        pd.read_csv(deba.data("raw/brusly_pd/brusly_pd_awards_2015-2020_byhand.csv"))
        .pipe(clean_column_names)
        .rename(
            columns={
                "l_name": "last_name",
...
if __name__ == "__main__":
    ...
    pprr = clean_pprr()
    award = clean_award()
    cprr.to_csv(deba.data("clean/cprr_brusly_pd_2020.csv"), index=False)
    pprr.to_csv(deba.data("clean/pprr_brusly_pd_2020.csv"), index=False)
    award.to_csv(deba.data("clean/award_brusly_pd_2021.csv"), index=False)
