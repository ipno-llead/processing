...
def rename_agency(df):
    df.loc[:, "agency"] = df.agency.str.strip().fillna("").str.replace(r"(\w)\.\s*(\w)\.", r"\1\2", regex=True)\
        ...
        .str.replace(r'^Se La Flood Protection Auth- E$',
                     'Southeast Louisiana Flood Protection Authority', regex=True)\
        ...
        .str.replace('Housing Authority Of NO', 'New Orleans Housing Authority', regex=False)\
        .str.replace(r'^Ebr', 'East Baton Rouge', regex=False)\
        .str.replace('City Park PD - NO', 'New Orleans City Park PD', regex=False)\
        .str.replace('Nd', 'nd', regex=False).str.replace(r'^(\w+)St', r'\1st', regex=True)\
        ...
        .str.replace(r'^LSP$', 'Louisiana State PD')
    return df
