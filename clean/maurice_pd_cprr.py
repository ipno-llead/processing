...
def clean_disposition(df):
    df.loc[:, 'disposition'] = df.disposition\
        .str.replace('found to be in ', '', regex=False)
    return df


def clean_action(df):
    df.loc[:, 'action'] = df.action\
        .str.replace(r'\(pening\)', '- pending', regex=True)
    return df


def clean():
    df = pd.read_csv(data_file_path('raw/maurice_pd/maurice_cprr_2020_2021_byhand.csv'))\
        ...
        .pipe(gen_uid, [
            ...
            'incident_year', 'incident_month', 'incident_day'], 'allegation_uid')
    return df
