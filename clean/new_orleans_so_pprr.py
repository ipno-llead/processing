...
def clean_division_desc(df):
    df.loc[:, 'division_desc'] = df.home_department_description.str.lower().str.strip()\
        .str.replace(r'  +', ' ', regex=True).str.replace(r'^security- ', '', regex=True)\
        .str.replace(r'civil division - (\w+)', r'civil \1', regex=True)\
        .str.replace('&', 'and', regex=False).str.replace('extended leave', '', regex=False)\
        .str.replace(' and processing', '', regex=False).str.replace('iad', 'internal affairs', regex=False)\
...
def clean_rank(df):
    df.loc[:, 'rank_desc'] = df.job_title_description.str.lower().str.strip()\
        ...
        .str.replace(r'hr |human resources |payroll |real estate |sanitation |'
                     ...
                     r' - judicial enforcement| -jto|reserve ', '', regex=True)\
        ...
        .str.replace(r'administrative assistant (.+)', 'administrative assistant', regex=True)\
        .str.replace(r'(.+) analyst', 'analyst', regex=True)
    return df.drop(columns='job_title_description')


def remove_incorrect_salaries(df):
    df.loc[:, 'salary'] = df.salary.astype(str)\
        .str.replace(r'^(\d{4})\..+', '', regex=True)
    return df


def clean_officer_inactive(df):
    df.loc[:, 'officer_inactive'] = df.position_status.str.lower().str.strip()\
        .str.replace('active', 'no', regex=False)
    return df.drop(columns='position_status')


def extract_officer_contract_status(df):
    df.loc[:, 'worker_category_description'] = df.worker_category_description.str.lower().str.strip()\
        .str.replace('temp', 'temporary', regex=False)\
        .str.replace('regular', 'permanent', regex=False)
    status = df.worker_category_description.str.extract(r'(temporary|permanent|volunteer)')
    df.loc[:, 'officer_contract_status'] = status[0]
    return df


def clean_employment_status(df):
    df.loc[:, 'officer_employment_status'] = df.worker_category_description\
        .str.replace(r'( ?temporary ?| ?permanent ?|volunteer| sheriff only)', '', regex=True)
    return df.drop(columns='worker_category_description')


def split_names(df):
    df.loc[:, 'payroll_name'] = df.payroll_name.str.lower().str.strip()\
