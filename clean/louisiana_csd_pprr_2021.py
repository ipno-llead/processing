...
from lib.clean import (
    clean_dates, clean_races, clean_sexes, parse_dates_with_known_format, clean_salaries,
    standardize_desc_cols, clean_names
)
...
def clean_demo():
    return pd.read_csv(data_file_path(
        'raw/louisiana_csd/louisiana_csd_pprr_demographics_2021.csv'
...
def clean_term():
    return pd.read_csv(data_file_path(
        'raw/louisiana_csd/louisiana_csd_pprr_terminations_2021.csv'
    )).pipe(clean_column_names)\
...
if __name__ == '__main__':
    demo = clean_demo()
    term = clean_term()
    demo.to_csv(data_file_path(
        'clean/pprr_demo_louisiana_csd_2021.csv'
    ), index=False)
    term.to_csv(data_file_path(
