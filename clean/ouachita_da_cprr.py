...
def clean_charges(df):
    df.loc[:, 'charges'] = df.charges\
        ...
        .str.replace('warrant issued for 1 count of simple battery', '1 count of simple battery')\
        .str.replace('prosecution pending in union parish for 1 count of simple battery', 
...
def extract_agency_and_department_desc(df):
    ...
    df.loc[:, 'department_desc'] = departments[0]\
        .str.replace('troof', 'troop', regex=False)
    return df


def clean_action(df):
    df.loc[:, 'action'] = df.action\
        .str.replace(' from lsp troof f for excessive use of force', '', regex=False)
    return df
...
if __name__ == '__main__':
    ...
    df.to_csv(data_file_path(
        'clean/cprr_ouachita_da_2021.csv'), index=False)
