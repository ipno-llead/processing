...
def clean_tracking_number(df):
    df.loc[:, 'tracking_number'] = df.ia_case_number.str.lower().str.strip()\
...
def clean_allegation(df):
    df.loc[:, 'allegation'] = df.allegation.str.lower().str.strip()\
        ...
        .str.replace(r'gen\. order 121 - unit usage', 'general order 121: unit usage', regex=True)\
        .str.replace('107 - misuse of equipment', 'general order 107: misuse of equipment', regex=False)
    return df
...
def clean_action(df):
    df.loc[:, 'action'] = df.discipline.str.lower().str.strip().fillna('')\
...
def clean_allegation_desc(df):
    df.loc[:, 'allegation_desc'] = df.notes.str.lower().str.strip()\
...
def split_rows_with_multiple_officers(df):
    df.loc[:, 'officer_name'] = df.officer_name.str.lower().str.strip()\
...
def split_and_clean_names(df):
    names = df.officer_name\
        .str.replace('officer name', '', regex=False)\
        .str.extract(r'(\w+) ?(\w+)? (\w+)$')
...
def clean():
    df = pd.read_csv(data_file_path('raw/abbeville_pd/abbeville_pd_cprr_2019_2021.csv'))\
        ...
        .pipe(gen_uid, [
            'uid', 'allegation', 'disposition', 'action', 
            'receive_year', 'receive_day', 'receive_month'], 'allegation_uid')
    return df
