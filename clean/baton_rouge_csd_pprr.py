...
def clean_rank_desc(df):
    df.rank_desc = (
        ...
        .str.replace(
            "college student intern/contract", "college student intern", regex=False
        )
        ...
        .str.replace(
            r"prog(rammer)?/?sys(tems)? ?analy(st)?",
...
def clean_17():
    df = pd.read_csv(
        deba.data("raw/baton_rouge_csd/baton_rouge_csd_pprr_2017.csv")
    ...
    df = (
        ...
        .pipe(
            set_values, {"data_production_year": "2017", "salary_freq": salary.YEARLY}
        )
...
def clean_19():
    df = pd.read_csv(
        deba.data("raw/baton_rouge_csd/baton_rouge_csd_pprr_2019.csv")
    ).pipe(clean_column_names)
    ...
    df = (
        ...
        .pipe(
            set_values, {"data_production_year": "2019", "salary_freq": salary.YEARLY}
        )
        .drop_duplicates()
        .pipe(clean_salaries, ["salary"])
        .pipe(clean_rank_desc)
...
if __name__ == "__main__":
    ...
    df17.to_csv(deba.data("clean/pprr_baton_rouge_csd_2017.csv"), index=False)
    df19.to_csv(deba.data("clean/pprr_baton_rouge_csd_2019.csv"), index=False)
