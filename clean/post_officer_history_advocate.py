...
def split_agency_column(df):
    data = (
        ...
        .str.strip()
        .str.extract(
            r"(\w+? ?\w+? ? ?\w+? ?\w+? ? ?\w+?) ?(?:(full-time|reserve|retired|part-time|deceased?) )? ? ?(\w{1,2}\/\w{1,2}\/\w{4}) ? ?(\w{1,2}\/\w{1,2}\/\w{4})? ?(termina?t?i?o?n?|(\w+)? ?(resignation)|(\w+)? ?(resig?n?e?d?)|retired)?"
        )
    )
...
def clean():
    df = (
        pd.read_csv(deba.data("ner/advocate_post_officer_history_reports.csv"))
        .pipe(drop_rows_missing_names)
        .drop(columns=["officer_name_1","officer_name_2", "officer_sex_1", "officer_sex_2"])
...
if __name__ == "__main__":
    df = clean()
    df.to_csv(deba.data("clean/advocate_post_officer_history.csv"), index=False)
