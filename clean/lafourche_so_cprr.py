...
import pandas as pd
import deba
from lib.clean import clean_sexes, standardize_desc_cols, clean_dates, clean_races, clean_names
...
def clean_receive_date(df):
    df.loc[:, "receive_date"] = df.date.astype(str).str.lower().str.strip()\
        .str.replace(r"^carry over for 2020$", "", regex=True)
    return df.drop(columns=["date"])

def extract_race_and_sex_columns(df):
    races = df.race_sex_office.str.lower().str.strip()\
        .str.extract(r"(^b|^w)")
    df.loc[:, "race"] = races[0].fillna("")\
...
def extract_disposition(df):
    dispositions = df.outcome.str.lower().str.strip()\
...
def clean_actions(df):
    df.loc[:, "action"] = df.outcome.str.lower().str.strip()\
        .str.replace(r"(\w+) $", r"\1", regex=True)\
        ...
        .str.replace(r"^resigned under investigation", "resignation while under investigation", regex=True)\
        .str.replace("refered", "referred", regex=False)\
        .str.replace(r"(^abandoned$|^handeled at the division$|no violations|^unfounded$|\bnot sustained\b|\bsustained\b)", "", regex=True)
    return df 
...
def clean():
    df = pd.read_csv(deba.data("raw/lafourche_so/lafourche_so_cprr_2019_2021.csv"))\
        ...
        .pipe(gen_uid, ["uid", "tracking_number", "allegation", "disposition", "action"], "allegation_uid")
    return df 
