...
def clean_employment_status(df):
    df.loc[:, 'employment_status'] = df.ft_or_pt_status.str.lower().str.strip()\
        .str.replace('ft', 'full time', regex=False).str.replace('pt', 'part time', regex=False)
...
def clean():
    df = pd.read_csv(data_file_path('raw/central_csd/central_csd_pprr_2014_2019.csv'))\
