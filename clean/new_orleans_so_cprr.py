...
action_lookup = [
    ...
    ['currently out due to covid-19/downgraded to a write-up',
     'currently out due to covid downgraded to a dm-1', 'out due to covid-19 downgraded to dm-1'],
...
def clean_allegations_20(df):
    df.loc[:, 'allegation'] = df.charges.str.strip()\
        ...
        .str.replace(r'ins?str?s?uctions? ?(from)? ?(a)? ?(sus?pp?erv?i?s?i?os?r?)? ?(of)? '
                     '?(or)? ?(higher)? ?(ranke?d?)? ?(sr?at?ff)? ?(mm?em?ber)?',
                     'instructions from a supervisor or higher ranked staff member/', regex=True)\
...
def extract_suspension_start_date(df):
    df.loc[:, 'suspension_start_date'] = df.action\
...
def extract_suspension_end_date(df):
    df.loc[:, 'suspension_end_date'] = df.action.str.lower().str.strip()\
        .str.replace(r'\breturn\b', 'returned', regex=True)
    dates = df.suspension_end_date.str.extract(r'(returned.+)')
    df.loc[:, 'suspension_end_date'] = dates[0]\
        .str.replace(r'returned ?(back)? ?(to)? ?(duty)? ?(on)? ?', '', regex=True)
    return df
