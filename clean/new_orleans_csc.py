...
def clean_suspension(df):
    df.loc[:, 'action_appealed'] = df.action.str.lower().str.strip()\
        ...
        .str.replace('suspension from working details for 60-day', 
                     '60-day suspension from working detail', regex=False)\
        .str.replace('detail privileges suspension for 12-months', 
                     '12-month suspension from detail privileges', regex=False)\
        .str.replace('suspension working details for 64-days', 
                     '64-day suspension from working detail', regex=False)\
...
def clean_on_docket(df):
    df.loc[:, 'motions'] = df.on_docket.str.lower().str.strip()\
        .str.replace(r'(\w+)---?(\w+)', r'\1-\2', regex=True)\
        .str.replace(r'(\w+) - (\w+)', r'\1-\2', regex=True)\
        ...
        .str.replace(r'motion for rehearing ?(and)? ?(request)? ?(for)? oral argument/?',
                     'motion for rehearing/request for oral argument', regex=True)\
        .str.replace('motion for rehearing and ', 'motion for rehearing/', regex=False)\
        .str.replace('request for clarification', 'motion for clarification', regex=False)\
        ...
        .str.replace('request for oral argument/motion for rehearing', 
                     'motion for rehearing/request for oral argument', regex=False)\
        ...
        .str.replace('withdrawal of motion to enforce decision', 'motion to withdraw', regex=False)\
        .str.replace(r'motion for summary disposition ?(and)? ?a? request ?(for)? oral argument',
                     'motion for summary disposition/oral argument', regex=True)\
...
def clean_final_disposition(df):
    df.loc[:, 'appeal_disposition'] = df.final_disposition.str.lower().str.strip()\
        .str.replace(r'(\w+) - (\w+)', r'\1/\2', regex=True)
...
def clean_last_name(df):
    df.loc[:, 'last_name'] = df.last_name.str.lower().str.strip().fillna('')\
...
def clean():
    df = pd.read_csv(data_file_path('raw/new_orleans_csc/new_orleans_csc_lprr_2000_2016.csv'))\
        .pipe(clean_column_names)\
        .drop(columns=[
            'department', 'final_disposition_docket',
            'final_disposition_docket_date', 'hearing_date_2'])\
        .rename(columns={
        ...
        .pipe(assign_agency)\
        .pipe(clean_dates, [ 
            'appeal_hearing_date', 'appeal_receive_date',
            'appeal_hearing_2_date', 'transcript_receive_date',
            'h_e_report_receive_date', 'appeal_disposition_date'])\
...
if __name__ == '__main__':
    ...
    df.to_csv(data_file_path(
        'clean/lprr_new_orleans_csc_2000_2016.csv'), index=False)
