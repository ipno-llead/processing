...
def clean_names(df):
    names = df.deputy.str.lower().str.strip().fillna('')\
        .str.extract(r'(\w+) ?(\w+)? (\w+)')
...
def clean():
    df = pd.read_csv(data_file_path('raw/acadia_so/cprr_acadia_so_2018_2021.csv'))\
        ...
        .pipe(clean_action)\
        .pipe(set_values, {
            'agency': 'acadia parish so'
        })\
        .pipe(gen_uid, ['agency', 'first_name', 'middle_name', 'last_name'])\
        .pipe(gen_uid, ['uid', 'allegation', 'action', 'receive_day', 'receive_month'], 'allegation_uid')
    return df
