...
def extract_parish_and_agency(df):
    ...
    df.loc[:, "agency"] = (
        ...
        .str.replace(
            r"^louisiana-state-university-pd$", "lsu-university-pd", regex=True
        )
...
def clean_conviction_dates(df):
    df.loc[:, "conviction_date"] = df.conviction_year.str.replace(
        r"Not available", "", regex=True
...
def assign_names(df):
    df.loc[
        df.name
        == "[Wald, Kevin](https://www.theadvocate.com/baton_rouge/news/article_789c9ab0-bdb8-11e6-b27f-bf5dc9d868ff.html)",
        "first_name",
    ] = "kevin"
    df.loc[
        df.name
    ...
    df.loc[
        df.name
        == "[Hill, Dennis](https://www.theadvocate.com/baton_rouge/news/article_1f96e08c-fe58-11e8-aa56-b39b05a03569.html)",
        "first_name",
    ] = "dennis"
    df.loc[
        df.name
    ...
    df.loc[
        df.name
        == "[Pope, Brian](https://www.theadvocate.com/acadiana/news/article_43f6d472-cd50-11eb-85a3-0b5678e393bd.html)",
        "first_name",
    ] = "brian"
    df.loc[
        df.name
    ...
    df.loc[
        df.name
        == "[Hart, Tianna](https://www.justice.gov/usao-edla/pr/former-deputy-sheriff-pleads-guilty-making-false-statement)",
        "first_name",
    ] = "tianna"
    df.loc[
        df.name
    ...
    df.loc[
        df.name
        == "[Core, Brad](https://www.wafb.com/2021/06/23/former-hammond-police-officer-pleads-guilty-child-sex-crimes/)",
        "first_name",
    ] = "brad"
    df.loc[
        df.name
    ...
    df.loc[
        df.name
        == "[Bahm, Brett](https://www.documentcloud.org/documents/23783741-st-tammany-washington-parish-police-convictions?responsive=1&title=1)",
        "first_name",
    ] = "brett"
    df.loc[
        df.name
    ...
    df.loc[
        df.name
        == "[Gary, Elijah](https://www.documentcloud.org/documents/23783718-plaquemines-parish-police-convictions?responsive=1&title=1)",
        "first_name",
    ] = "gary"
    df.loc[
        df.name
...
def clean():
    df = (
        pd.read_csv(
            deba.data(
                "raw/the_advocate/criminal_convictions_data_joseph_cranney_the_advocate.csv"
            )
        )
