...
def split_names(df):
    names = df.title_of_report.str.lower()\
                .str.strip()\
                .str.replace(r"(use of force:? |\[|\]|(\w+\/\w+\/\w+|-|\w+\-\w+\-\w+|\(|\)|S2888))", "", regex=True)\
                ...
                .str.extract(r"^(cpl|det|sgt|dy|lt|deputy|detective|\bpl|corporal|sergeant|cp)?\.? ?(\w+)?\.? ?(\w{1})?\.? (\w+)")
    ranks = {"sgt": "sergeant", "cpl": "corporal", "det": "detective", "dy": "deputy", "lt": "lieutenant"}
...
def clean_uof_desc(df):
    df.loc[:, "use_of_force_description"] = df.force_used.str.lower().str.strip()\
        ...
        .str.replace(r"k-9 ?-? surrendere?d to warnings", r"surrendered to k-9 warnings", regex=True)\
...
def clean_uof_effective(df):
    df.loc[:, "use_of_force_effective"] = df.effectiveness_of_force_used.str.lower().str.strip()\
        .str.replace(r"#", "", regex=False)\
        .str.replace(r";", "; ", regex=False)\
...
def clean():
    df = (pd.read_csv(deba.data("raw/baton_rouge_so/baton_rouge_so_uof_2020.csv"))\
        ...
        .pipe(gen_uid, ["report_date", "uid", "use_of_force_description", "use_of_force_effective"], "uof_uid")
    )
