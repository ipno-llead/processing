...
def stack_disposition_rows(df):
    ...
    combined = df.iloc[:, 2].fillna('').str.strip().str.cat(
    ...
    ).stack().reset_index().iloc[:, [0, 1, 3]].rename(columns={0: 'charges'})

    # split "charges" into "allegation" and "disposition"
    df = pd.concat(
        [df, df.charges.str.extract(r'(.*?)(?:%s)?$' % disp_pattern)],
        axis=1,
    ).drop(columns=['charges']).rename(columns={0: 'allegation', 1: 'disposition'})
...
def clean_tracking_number(df):
    df.loc[:, 'tracking_number'] = df.tracking_number.str.strip().str.upper()\
...
def stack_name_rows(df):
    return pd.DataFrame(df.iloc[:, 4:])\
        ...
        .stack().reset_index()\
        .drop(columns=['level_4']).rename(columns={0: 'allegation'})
...
def split_names(df):
    df = pd.concat(
        [
            df,
            df.officer_name.str.strip().str.lower()
            .str.extract(r'([^ ]+),? (.+?)(?: (jr|sr)\.)?$'),
        ],
        axis=1,
    ).drop(columns=['officer_name'])\
        .rename(columns={0: 'last_name', 1: 'first_name', 2: 'suffix'})
    df.loc[:, 'last_name'] = df.last_name.str.cat(df.suffix.fillna(''), sep=' ')\
        .str.strip()
...
def make_disposition_categorical(df):
    df.loc[:, 'disposition'] = df.disposition.astype(pd.CategoricalDtype(categories=[
        'SUSTAINED',
        'NOT SUSTAINED',
        'UNFOUNDED',
        'EXONERATED',
...
def clean_cprr(disposition_file, name_file, year):
    ...
    df = df.merge(
        name_df[[
            'tracking_number', 'uid', 'first_name', 'last_name', 'middle_name', 'middle_initial', 'complainant_name'
        ]].drop_duplicates(),
        on='tracking_number', how='left',
    )
    df = pd.concat([
        df,
        name_df[~name_df.tracking_number.isin(df.tracking_number.unique())]
    ])
...
def clean_codebook():
    df = pd.read_csv(data_file_path(
        'raw/shreveport_pd/shreveport_codebook.csv'
    ), names=['name', 'code'])
