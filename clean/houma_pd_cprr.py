...
def clean_allegations(df):
    df.loc[:, 'allegation'] = df.allegation.str.lower().str.strip()\
        ...
        .str.replace(r'^conduct unbecoming$', 'conduct unbecoming an officer', regex=True)
    return df
...
def extract_action(df):
    df.loc[:, 'action'] = df.inv_disposition.str.lower().str.strip()\
        ...
        .str.replace(r'((no[tn])?\-? ?sust[aä]n?ined|(all officers)? ?exonerated|unfounded|'
...
def split_and_clean_investigator18(df):
    investigators = df.i_a_investigator.str.lower().str.strip()\
        .str.extract(r'(sgt)\.? (\w+) (\w+)')
...
def clean_tracking_number(df):
    df.loc[:, 'tracking_number'] = df.pd_case_number.str.lower().str.strip()\
...
def clean21():
    df = pd.read_csv(data_file_path('raw/houma_pd/houma_pd_cprr_2019_2021.csv'))\
        ...
        .pipe(gen_uid, ['uid', 'tracking_number', 'case_number', 'allegation', 'action'], 'allegation_uid')
    return df


def clean18():
    df = pd.read_csv(data_file_path('raw/houma_pd/houma_pd_cprr_2017_2018.csv'))\
        .pipe(clean_column_names)\
        ...
        .pipe(set_values, {'agency': 'Houma PD'})\
        .pipe(gen_uid, ['agency', 'first_name', 'last_name'])\
        .pipe(gen_uid, ['uid', 'allegation', 'action', 'case_number', 'tracking_number'], 'allegation_uid')
    return df
