...
def assign_department_desc(df):
    dept_df = pd.read_csv(data_file_path(
        "dropbox/st_tammany_so/st_tammany_department_codes_tabula.csv"
    ))
...
def clean():
    df = pd.concat([
        pd.read_csv(data_file_path(
            'dropbox/st_tammany_so/st_tammany_so_cprr_2011-2020_tabula.csv')),
        pd.read_csv(data_file_path(
            'dropbox/st_tammany_so/st_tammany_so_cprr_2020-2021_tabula.csv'))
    ])
