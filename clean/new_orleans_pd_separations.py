...
def clean_rank_desc22(df):
    df.loc[:, "rank_desc"] = df.job_title.str.lower().str.strip()\
        .str.replace("police", "", regex=True).str.replace(r"(\w+) +(\w+)", r"\1 \2", regex=True)\
        .str.replace(r"(\w+) +$", r"\1", regex=True).str.replace(r"^ +(\w+)", r"\1", regex=True)\
        .str.replace(r"(\w+) senior$", r"\1", regex=True).str.replace(r"supt\. of", "superintendant", regex=True)
    return df.drop(columns=["job_title"])

def correct_dates_2022(df):
    df.loc[:, "hire_date"] = df.hire_date.str.replace(r"12\/13\/015", r"12/13/2015", regex=True)
    return df 


def clean_left_reason22(df):
    df.loc[:, "left_reason"] = df.reason.str.lower().str.strip()\
        .str.replace(r"(\w+) +$", r"\1", regex=True)\
...
def clean_left_reason_desc22(df):
    df.loc[:, "left_reason_desc"] = df.reason_1.str.lower().str.strip()\
...
def clean_years_of_service(df):
    df.loc[:, "years_of_service"] = df.years_of_service.str.lower().str.strip()\
...
def clean22():
    df = pd.read_csv(deba.data("raw/new_orleans_pd/nopd_cprr_separations_2022.csv"))\
        .pipe(clean_column_names)\
        ...
        .pipe(standardize_desc_cols, ["left_reason", "left_reason_desc", "years_of_service", "employee_id", 
        "rank_desc", "left_date"])\
        .pipe(set_values, {"agency": "New Orleans PD"})\
        .pipe(gen_uid, ["first_name", "last_name", "agency"])\
        .pipe(gen_uid, ["left_reason", "left_reason_desc", "left_date"], "separation_uid")
    return df 
