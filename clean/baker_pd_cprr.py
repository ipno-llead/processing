...
def split_names(df):
    names = df.officer_name.str.lower().str.strip()\
        .str.extract(r'(\w+)\. (\w+) (\w+)')
...
def join_disposition_columns(df):
    df.loc[:, 'disposition'] = df.sustained.str.lower().str.strip().fillna('')\
        .str.cat(df.not_sustained.str.lower().str.strip().fillna(''))

    df.loc[:, 'disposition'] = df.disposition\
        .str.replace(r'resigned his employment prior to investigation being performed\.',
                     'resigned', regex=True)\
...
def clean_allegation(df):
    df.loc[:, 'allegation'] = df.allegations_complaints.str.lower().str.strip()\
        .str.replace(r'\.', '', regex=True)\
        .str.replace(r'\bdept\b', 'department', regex=True)\
        .str.replace(r'\badmin\b', 'administration', regex=True)\
        .str.replace('cooperation with other police agencies - (failure to)',
                     'failure to cooperate with other police agencies', regex=False)\
        .str.replace('zcooperation', 'cooperation', regex=False)\
        .str.replace('presenting statement of fact - (falsified)', 
                     'false presentation of statment of fact', regex=False)\
        .str.replace('use of force - (excessive)', 
                     'excessive use of force', regex=False)
    return df.drop(columns='allegations_complaints')


def clean():
    df = pd.read_csv(data_file_path('raw/baker_pd/baker_pd_cprr_2018_2020.csv'))\
...
if __name__ == '__main__':
    df = clean()
    df.to_csv(data_file_path('clean/cprr_baker_pd_2018_2020.csv'), index=False)
