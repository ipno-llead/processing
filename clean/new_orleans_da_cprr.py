...
from lib.clean import clean_sexes, standardize_desc_cols, clean_dates, clean_races, float_to_int_str, clean_names
import deba
...
def split_names(df):
    ...
    df.loc[:, "last_name"] = df.last_name.fillna("").str.cat(df.suffix.fillna(""), sep=" ")
    return df.drop(columns=["suffix", "officer"])


def clean_complainant_type(df):
    df.loc[:, "complainant_type"] = df.incident_type.str.lower().str.strip()\
        .str.replace(" initiated", "", regex=False)
    return df


def clean_allegation(df):
    df.loc[:, "allegation"] = df.rule.str.lower().str.strip().str.cat(df.paragraph.str.lower().str.strip(), sep="; ")

    df.loc[:, "allegation"] = df.allegation\
        .str.replace(r"\bprof\b", "professionalism", regex=True)\
...
def clean_disposition(df):
    df.loc[:, "disposition"] = df.allegation_finding.str.lower().str.strip().fillna("")\
        ...
        .str.replace(r"sustained; rui - resign", "sustained; resigned while under investigation", regex=True)\
        .str.replace(r"^rui ?- ?resign(ed under investigation)?$", "resigned while under investigation", regex=True)\
        .str.replace(r"^susatined$", "sustained", regex=True)
    return df.drop(columns=["allegation_finding"])

def clean_unit_sub_desc(df):
    df.loc[:, "unit_sub_desc"] = df.unit_additional_details_of_complainant.str.lower().str.strip()\
...
def clean_unit_desc(df):
    df.loc[:, "unit_desc"] = df.unit_of_complainant.str.lower().str.strip()\
        .str.replace(r"(\w+) -(\w+)", r"\1 \2", regex=True)\
        .str.replace(r"(\w+)  (\w+)", r"\1 \2", regex=True)\
...
def clean_division_desc(df):
    df.loc[:, "division_desc"] = df.division_of_complainant.str.lower().str.strip()\
        .str.replace(r"^pib ?(- public integrity bureau)?", "public integrity bureau", regex=True)\
        .str.replace("&", "and", regex=False)\
...
def clean_department_desc(df):
    df.loc[:, "department_desc"] = df.bureau_of_complainant.str.lower().str.strip()\
...
def clean_employment_status(df):
    df.loc[:, "employment_status"] = df.employment_status.str.lower().str.strip()\
        ...
        .str.replace(r"(\w+)  (\w+)", r"\1 \2", regex=True)\
        .str.replace(R"^rui - resigned under investigation$", "resigned under investigation", regex=True)
    return df 

def clean_work_shift(df):
    df.loc[:, "work_shift"] = df.shift_of_complainant.str.lower().str.strip()\
        .str.replace(r"(.+) - (.+)", r"\1-\2", regex=True)\
        .str.replace(r"^\b(?:(\w{1,2}):?(\w{1,2} ?[pmam]) ?)", r"between \1:\2", regex=True)
    return df.drop(columns=["shift_of_complainant"])


def clean_allegation_desc(df):
    df.loc[:, "allegation_desc"] = df.allegation_desc.str.lower().str.lower()\
        .str.replace(r"(.+)\((\w+)\)", r"\1 (\2)", regex=True)
    return df 


def clean():
    df = pd.read_csv(deba.data("raw/new_orleans_pd/new_orleans_da_cprr_giglio_2021.csv"))\
        .pipe(clean_column_names)\
        .drop(columns=["complaint_disposition", "complaint_classification"])\
        .rename(columns={"complaint_tracking_number": "tracking_id", 
        "linked_item_number": "item_number", "date_complaint_occurred": "occur_date", 
        "date_complaint_received_by_nopd_pib": "receive_date", "date_complaint_investigation_complete": "investigation_complete_date",
        "working_status_of_complainant": "employment_status",
        "relevant_policy": "allegation_desc", "officer_firstname": "first_name", 
        "officer_lastname": "last_name", "officer_race_ethnicity": "race", "officer_years_of_service": "years_of_service",
        "complainant_gender": "citizen_sex", "complainant_age": "citizen_age",
        "officer_age": "age", "officer_gender": "sex", "complainant_ethnicity": "citizen_race"})\
        .pipe(split_names)\
        ...
        .pipe(standardize_desc_cols, ["complainant_type", "tracking_id", "item_number", "investigation_status", "work_shift"])\
        .pipe(clean_sexes, ["sex", "citizen_sex"])\
        .pipe(clean_dates, ["occur_date", "receive_date", "investigation_complete_date"])\
        .pipe(clean_names, ["first_name", "last_name"])\
        .pipe(clean_races, ["race", "citizen_race"])\
        .pipe(set_values, {"agency": "New Orleans PD"})\
        .pipe(gen_uid, ["first_name", "last_name", "agency"])\
        .pipe(gen_uid, ["allegation", "allegation_desc", "disposition", "tracking_id", "uid"], "allegation_uid")\
        .drop(columns=["bureau_of_complainant", "division_of_complainant", "unit_of_complainant", "unit_additional_details_of_complainant"])\
