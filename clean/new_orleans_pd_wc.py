...
def venezia():
    df = (
        pd.read_csv(deba.data("raw/new_orleans_pd/venezia_martin.csv"))
        .pipe(assign_demo)
        .pipe(set_values, {"agency": "new-orleans-pd"})
        .pipe(gen_uid, ["first_name", "last_name", "agency"])
        .pipe(
            gen_uid,
            ["uid", "incident_date", "allegation", "disposition"],
            "allegation_uid",
        )
    )
    return df
...
def dillmann():
    df = (
        pd.read_csv(deba.data("raw/new_orleans_pd/dillmann_john.csv"))
        .pipe(assign_demo)
        .pipe(assign_rank)
        .pipe(filter_agency)
        .pipe(gen_uid, ["first_name", "last_name", "agency"])
