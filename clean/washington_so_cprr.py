...
def extract_action_from_disposition(df):
    df.loc[:, "action"] = (
        ...
        .str.replace(
            "susp 2 wks. eventually resigned.",
            "2-week suspension; eventually resigned",
            regex=False,
        )
        .str.replace(
            "turned over to lsp & u.s. atty's office: arrested - criminal charges pending in 22nd jdc",
            r"case turned over to the louisiana state police and u.s attorney's office;"
            r" arrested; criminal charges are pending in the 22nd judicial district court",
            regex=True,
        )
        .str.replace(r"\/", "; ", regex=True)
    )
...
def split_names(df):
    names = (
        df.name_of_accused_deputy_list_separately_if_more.str.lower()
        .str.strip()
        .str.replace(r"\.", "", regex=True)
...
def clean():
    df = (
        pd.read_csv(bolo.data("raw/washington_so/washington_so_cprr_2015_2020.csv"))
...
if __name__ == "__main__":
    df = clean()
    df.to_csv(bolo.data("clean/cprr_washington_so_2015_2020.csv"), index=False)
