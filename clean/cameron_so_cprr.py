...
def clean_20():
    df = (
        pd.read_csv(dirk.data("raw/cameron_so/cameron_so_cprr_2020.csv"))
...
def clean_19():
    df = (
        pd.read_csv(dirk.data("raw/cameron_so/cameron_so_cprr_2015_2019.csv"))
        .pipe(clean_column_names)
        .pipe(split_name)
        .pipe(clean_names, ["first_name", "last_name"])
...
if __name__ == "__main__":
    df_20 = clean_20()
    df_19 = clean_19()
    df_20.to_csv(dirk.data("clean/cprr_cameron_so_2020.csv"), index=False)
    df_19.to_csv(dirk.data("clean/cprr_cameron_so_2015_2019.csv"), index=False)
