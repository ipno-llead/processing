...
def parse_birthdate(df):
    df.loc[:, "birth_date"] = pd.to_datetime(
        df.birth_date.str.replace(r"^(\d-.+)", r"0\1", regex=True).str.replace(
            r"^`$", "", regex=True
        ),
        errors="coerce",
        format="%d-%b-%y",
...
def split_rows(df):
    df.loc[:, "hire_date_raw"] = (
        df.hire_date_raw.fillna("")
        .str.lower()
    ...
    for _, row in df.iterrows():
        ...
        rec["hire_date_raw"] = (
            row.hire_date_raw[0]
            if len(row.hire_date_raw) == 1
            else row.hire_date_raw[1]
        ).strip()
        rec["salary_raw"] = (
            row.salary_raw[0] if len(row.salary_raw) == 1 else row.salary_raw[1]
        ).strip()
        records.append(rec)
...
def extract_employment_status(df):
    ...
    df.loc[df.hire_date_raw.str.match(r"full time"), "employment_status"] = "full-time"
    df.loc[df.hire_date_raw.str.match(r"reserve"), "employment_status"] = "reserve"
    df.loc[
        df.hire_date_raw.str.match(r"^(part time|pt)"), "employment_status"
    ] = "part-time"
    df.loc[
        df.rank_desc.fillna("").str.lower().str.match(r"part-time"), "employment_status"
    ] = "part-time"
    df.loc[:, "rank_desc"] = (
        df.rank_desc.fillna("").str.lower().str.replace(r"part-time ", "", regex=False)
    )
    return df
...
def extract_rank(df):
    for rank in ["officer", "sergeant", "captain", "detective", "deputy chief"]:
        df.loc[
            df.rank_desc.isna() & (df.hire_date_raw.str.match(r".*%s" % rank)),
            "rank_desc",
        ] = rank
    return df
...
def extract_pay_effective_date(df):
    ...
    df.loc[
        ~df.salary_raw.str.match(pat), "pay_effective_date"
    ] = df.salary_raw.str.extract(r"(?:.* |^)(\d{4}).+")[0]
    return df


def extract_salary(df):
    df.loc[:, "salary"] = clean_salary(
        df.salary_raw.str.replace("$1 raise", "$10.50", regex=False).str.extract(
            r"(\$\d{1,2}(?:\.\d{2})?)"
        )[0]
    )
...
def clean():
    return (
        pd.read_csv(
            deba.data("raw/grand_isle/grand_isle_pd_pprr_2021_byhand.csv"),
            encoding="latin-1",
        )
        .pipe(clean_column_names)
        .rename(columns={"hire_date": "hire_date_raw", "hourly_salary": "salary_raw"})
...
if __name__ == "__main__":
    ...
    df.to_csv(deba.data("clean/pprr_grand_isle_pd_2021.csv"), index=False)
