...
def initial_processing():
    df = pd.read_csv(
        bolo.data("raw/ipm/new_orleans_pd_cprr_allegations_1931-2020.csv"),
        escapechar="\\",
    )
...
def combine_citizen_columns(df):
    ...
    citizens = (
        df[citizen_cols]
        .dropna(how="all")
        .drop_duplicates(subset=["allegation_primary_key", "citizen_primary_key"])
    ...
    for idx, frame in citizens.groupby(level=0):
        ...
        for _, row in frame.iterrows():
            citizen = [
                x
                for x in [row.citizen_race, row.citizen_sex]
    ...
    df.loc[:, "citizen"] = df.allegation_primary_key.map(
        lambda x: citizen_dict.get(x, "")
    )
    df = (
        df.drop(columns=["citizen_primary_key", "citizen_sex", "citizen_race"])
        .drop_duplicates()
...
def clean_complainant_type(df):
    df.loc[:, "complainant_type"] = (
        ...
        .str.replace(r" complain(t|ant)$", "", regex=True)
        .str.replace(r"civilian", "citizen", regex=True)
        .str.replace(r" offi$", " office", regex=True)
        .str.replace("rank", "nopd employee", regex=False)
        .str.replace("known", "", regex=False)
    )
...
def clean_allegation(df):
    df.loc[:, "allegation"] = (
        ...
        .str.strip()
        .str.replace(
            "no violation was observed to have been committed by officer/employee",
            "",
            regex=False,
        )
        ...
        .str.replace(
            "paragraph 03: cleanliness of department property",
            "paragraph 03: cleanliness of department vehicles",
            regex=False,
        )
        .str.replace(
            "paragraph 02 - authorized operator of department property",
            "paragraph 02: authorized operator of department vehicle",
            regex=False,
        )
        .str.replace(
            "paragraph 14 - social networking",
            "paragraph 13: social networking",
            regex=False,
        )
        .str.replace(
            "paragraph 04,05 - accepting, giving anything of value",
            "paragraph 04: accepting, giving anything of value",
            regex=False,
        )
        .str.replace(
            "paragraph 11 - interfering with investigations",
            "paragraph 13: interfering with investigations",
            regex=False,
        )
        .str.replace(
            "no violation was observed to have been committed by officer/employee",
        ...
        .str.replace(
            r"paragraph 09 - use of alcohol/(drugs)? ?on-?duty",
            "paragraph 08: use of alcohol/drugs on duty",
            regex=True,
        )
        .str.replace(
            r"(paragraph 10 - use of alcohol/drugs offduty|paragraph 10 - use of alcohol/off-duty)",
            "paragraph 09: use of alcohol/drugs off duty",
            regex=True,
        )
        .str.replace(
            "paragraph 08 - use of drugs/substance abuse testing",
        ...
        .str.replace(
            "to wit paragraph 01 and paragraph 08",
            "to wit paragraph 01; paragraph 08",
            regex=False,
        )
        .str.replace(r"paragraph (\d+) (\w+)", r"paragraph \1: \2", regex=True)
        .str.replace(
            r"paragraph 12,?1?3? - use of t[oa]bacco",
            "paragraph 11: use of tobacco",
            regex=True,
        )
        .str.replace(
            r"(^paragraph 02: abuse of position$|^paragraph 01: professionalism$|"
            ...
            r"rule 03: moral conduct; \1",
            regex=True,
        )
        .str.replace(
            r"(paragraph 01: adherence to law|paragraph 02: courtesy|"
            ...
            r"rule 02: moral conduct; \1",
            regex=True,
        )
        .str.replace(
            r"(paragraph 01: reporting for duty|paragraph 02: instructions from an authoritative source|"
            ...
            r"rule 04: performance of duty; \1",
            regex=True,
        )
        .str.replace(
            r"(paragraph 01: fictitious illness or injury|paragraph 02: associations|"
            ...
            r"paragraph 15: rewards)",
            r"rule 05: restricted activities; \1",
            regex=True,
        )
        .str.replace(
            r"(paragraph 01: security of records|paragraph 02: false or inaccurate reports|"
            ...
            r"rule 06: offical information; \1",
            regex=True,
        )
        .str.replace(
            r"(paragraph 01: use of department property|"
            ...
            r"paragrpah 07: surrending department property)",
            r"rule 07: department property; \1",
            regex=True,
        )
        .str.replace(
            r"(paragraph 01: rules of procedures|paragraph 0?2: effective|"
            r"paragraph 03: violations)",
            r"rule 01: operation manuals; \1",
            regex=True,
        )
    )
    return df
...
def discard_allegations_with_same_description(df):
    finding_cat = pd.CategoricalDtype(
        categories=[
            "sustained",
            "exonerated",
...
def clean_investigating_unit(df):
    df.loc[:, "investigating_unit"] = (
        df.assigned_unit.str.lower()
        .str.strip()
        .str.replace("un-assigned", "", regex=False)
        .str.replace("administrative", "administration", regex=False)
        .str.replace(r"serv\.", "service", regex=True)
        .str.replace(r"fob ?-? ?|igo ?-? ?|", "", regex=True)
        .str.replace(r"isb ?-? ?|msb ?-? ?", "", regex=True)
        .str.replace(r"invest\.", "investigative", regex=True)
        .str.replace(r"indep\.", "independent", regex=True)
...
def clean_investigation_complete_date(df):
    df.loc[:, "investigation_complete_date"] = df.completed_date.str.replace(
        r"(\d{1}):(\d{1})(\d{1})", "", regex=True
    )
...
def clean_investigating_department(df):
    df.loc[:, "investigating_department"] = (
        ...
        .str.strip()
        .str.replace(
            r"nopd officer|no longer employed| bureau|unkno?wo?n ?(bureau)?",
            "",
            regex=True,
        )
        ...
        .str.replace(
            r"^public integrity$", "public integrity/internal affairs", regex=True
        )
    ...
    return (
        df.drop(
            columns=[
                "all_findings",
                "allegation_1",
                "allegation_alert_processed",
                "allegation_alert_processed_date",
                "allegation_class_1",
                "allegation_directive",
                "allegation_final_disposition",
                "allegation_final_disposition_date",
                "allegation_finding",
                "allegation_finding_date",
                "assigned_date",
                "citizen_age",
                "citizen_num_shots",
                "county",
                ...
                "officer_type",
                "officer_unknown_id",
                "officer_years_exp_at_time_of_uof",
        ...
        .pipe(float_to_int_str, ["officer_primary_key", "allegation_primary_key"])
...
if __name__ == "__main__":
    df = clean()
    df.to_csv(bolo.data("clean/cprr_new_orleans_pd_1931_2020.csv"), index=False)
