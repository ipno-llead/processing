...
def clean18():
    df = pd.read_csv(dirk.data("raw/baton_rouge_so/baton_rouge_so_cprr_2018.csv"))
...
def clean20():
    df = pd.read_csv(dirk.data("raw/baton_rouge_so/baton_rouge_so_cprr_2016-2020.csv"))
    df = clean_column_names(df)
    df.columns = [
        "tracking_number",
        "name",
...
if __name__ == "__main__":
    df18 = clean18()
    df20 = clean20()
    df18.to_csv(dirk.data("clean/cprr_baton_rouge_so_2018.csv"), index=False)
    df20.to_csv(dirk.data("clean/cprr_baton_rouge_so_2016_2020.csv"), index=False)
