...
def extract_sex(df):
    df.loc[:, 'sex'] = df.race_gender.str.lower().str.strip()\
...
def clean_race(df): 
    df.loc[:, 'race'] = df.race_gender.str.lower().str.strip()\
        .str.replace('wm', 'w/m', regex=False)\
...
def clean():
    df = pd.read_csv(data_file_path('raw/gonzales_pd/gonzales_pd_pprr_2010_2021.csv'))\
