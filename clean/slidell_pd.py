...
from lib.clean import clean_names, clean_races, clean_salaries, clean_sexes, standardize_desc_cols
...
def realign09(df):
    ...
    return df.dropna(how='all', subset=['rank_desc', 'salary_date', 'hire_date', 'salary', 'salary_freq'])
...
def clean_rank(df):
    ...
    unincluded_vals = df.loc[(df.rank_desc != '') & ~df.rank_desc.isin(categories), 'rank_desc'].unique()
    if len(unincluded_vals) > 0:
        raise ValueError('rank values not found in categories: %s' % unincluded_vals)
    df.loc[:, 'rank_desc'] = df.rank_desc.astype(pd.CategoricalDtype(categories, ordered=True))
    return df
...
def clean_csd_pprr():
    return pd.read_csv(data_file_path('raw/slidell_pd/slidell_csd_pprr_2010_2019.csv'))\
        .dropna(how='all')\
        .pipe(clean_column_names)\
        .rename(columns={
            'frequency': 'salary_freq',
