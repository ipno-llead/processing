...
if __name__ == "__main__":
    ...
    complaints = rearrange_allegation_columns(pd.concat([cprr20, cprr14], axis=0))
    events_df = fuse_events(pprr, cprr20, cprr14)
    events_df = rearrange_event_columns(pd.concat([post_event, events_df]))
    personnels.to_csv(deba.data("fuse_agency/per_scott_pd.csv"), index=False)
    events_df.to_csv(deba.data("fuse_agency/event_scott_pd.csv"), index=False)
    complaints.to_csv(deba.data("fuse_agency/com_scott_pd.csv"), index=False)
    post.to_csv(deba.data("fuse_agency/post_scott_pd.csv"), index=False)
