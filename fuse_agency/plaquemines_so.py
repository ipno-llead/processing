...
if __name__ == "__main__":
    ...
    per_df.to_csv(deba.data("fuse_agency/per_plaquemines_so.csv"), index=False)
    com = rearrange_allegation_columns(pd.concat([cprr19, cprr20], axis=0))
    com.to_csv(deba.data("fuse_agency/com_plaquemines_so.csv"), index=False)
    events_df = rearrange_event_columns(pd.concat([events_df, post_event]))
    events_df.to_csv(deba.data("fuse_agency/event_plaquemines_so.csv"), index=False)
    post.to_csv(deba.data("fuse_agency/post_plaquemines_so.csv"), index=False)
