...
if __name__ == "__main__":
    ...
    complaints_df = rearrange_allegation_columns(pd.concat([cprr_20, cprr_14, cprr_08], axis=0))
    event_df = fuse_events(cprr_20, cprr_14, cprr_08, pprr)
    event_df = rearrange_event_columns(pd.concat([event_df, post_event]))
    event_df.to_csv(deba.data("fuse_agency/event_hammond_pd.csv"), index=False)
    personnel_df.to_csv(deba.data("fuse_agency/per_hammond_pd.csv"), index=False)
    complaints_df.to_csv(deba.data("fuse_agency/com_hammond_pd.csv"), index=False)
    post.to_csv(deba.data("fuse_agency/post_hammond_pd.csv"), index=False)
