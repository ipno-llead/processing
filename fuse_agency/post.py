...
if __name__ == "__main__":
    ...
    cprr23 = pd.read_csv(deba.data("match/cprr_post_decertifications_4_18_2023.csv"))
    event_df = fuse_events(post, cprr, cprr23)
    event_df = rearrange_event_columns(event_df)
    per_df = fuse_personnel(post, cprr, cprr23)
    per_df = rearrange_personnel_columns(per_df)
    allegation_df = rearrange_allegation_columns(pd.concat([cprr, cprr23]))
