...
if __name__ == "__main__":
    ...
    complaints = rearrange_allegation_columns(pd.concat([cprr21, cprr14], axis=0))
    event = fuse_events(cprr21, post, cprr14, cprr04)
    complaints.to_csv(deba.data("fuse_agency/com_benton_pd.csv"), index=False)
    per.to_csv(deba.data("fuse_agency/per_benton_pd.csv"), index=False)
    event.to_csv(deba.data("fuse_agency/event_benton_pd.csv"), index=False)
