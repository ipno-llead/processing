...
if __name__ == "__main__":
    ...
    post_event = pd.read_csv(dirk.data("match/post_event_gonzales_pd_2010_2021.csv"))
    events_df = rearrange_event_columns(pd.concat([post_event, fuse_events(pprr)]))
    rearrange_personnel_columns(pprr).to_csv(
        dirk.data("fuse/per_gonzales_pd.csv"), index=False
    )
    events_df.to_csv(dirk.data("fuse/event_gonzales_pd.csv"), index=False)
