...
if __name__ == "__main__":
    ...
    post_event = pd.read_csv(dirk.data("match/post_event_slidell_pd_2020.csv"))
    events_df = rearrange_event_columns(pd.concat([post_event, fuse_events(pprr_csd)]))
    fuse_personnel(pprr_csd).to_csv(dirk.data("fuse/per_slidell_pd.csv"), index=False)
    events_df.to_csv(dirk.data("fuse/event_slidell_pd.csv"), index=False)
