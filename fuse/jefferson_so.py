...
if __name__ == "__main__":
    ...
    post_event = pd.read_csv(dirk.data("match/post_event_jefferson_so_2020.csv"))
    personnel_df = fuse_personnel(pprr)
    events_df = rearrange_event_columns(post_event)
    rearrange_personnel_columns(pprr).to_csv(
        dirk.data("fuse/per_jefferson_so.csv"), index=False
    )
    events_df.to_csv(dirk.data("fuse/event_jefferson_so.csv"), index=False)
