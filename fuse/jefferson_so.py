...
if __name__ == "__main__":
    pprr = pd.read_csv(
        data_file_path("clean/pprr_jefferson_so_2020.csv"))
    post_event = pd.read_csv(data_file_path(
        "match/post_event_jefferson_so_2020.csv"))
    ...
    rearrange_personnel_columns(pprr).to_csv(data_file_path(
        "fuse/per_jefferson_so.csv"), index=False)
    events_df.to_csv(data_file_path(
        "fuse/event_jefferson_so.csv"), index=False)
