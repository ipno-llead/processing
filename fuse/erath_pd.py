...
if __name__ == "__main__":
    cprr = pd.read_csv(dirk.data("match/cprr_erath_pd_2018_2020.csv"))
    agency = cprr.agency[0]
    post = load_for_agency(agency)
    per = fuse_personnel(cprr, post)
    com = rearrange_allegation_columns(cprr)
    event = fuse_events(cprr, post)
    event.to_csv(dirk.data("fuse/event_erath_pd.csv"), index=False)
    com.to_csv(dirk.data("fuse/com_erath_pd.csv"), index=False)
    per.to_csv(dirk.data("fuse/per_erath_pd.csv"), index=False)
