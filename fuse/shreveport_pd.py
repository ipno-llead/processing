...
if __name__ == "__main__":
    cprr = pd.read_csv(dirk.data("match/cprr_shreveport_pd_2018_2019.csv"))
    agency = cprr.agency[0]
    post = load_for_agency(agency)
    event_df = fuse_events(cprr, post)
    per = fuse_personnel(cprr, post)
    com = rearrange_allegation_columns(cprr)
    per.to_csv(dirk.data("fuse/per_shreveport_pd.csv"), index=False)
    com.to_csv(dirk.data("fuse/com_shreveport_pd.csv"), index=False)
    event_df.to_csv(dirk.data("fuse/event_shreveport_pd.csv"), index=False)
