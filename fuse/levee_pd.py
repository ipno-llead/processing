...
if __name__ == "__main__":
    cprr = pd.read_csv(bolo.data("match/cprr_levee_pd.csv"))
    agency = cprr.agency[0]
    post = load_for_agency(agency)
    event_df = fuse_events(cprr, post)
    event_df.to_csv(bolo.data("fuse/event_levee_pd.csv"), index=False)
    complaint_df = rearrange_allegation_columns(cprr)
    complaint_df.to_csv(bolo.data("fuse/com_levee_pd.csv"), index=False)
    fuse_personnel(post, cprr).to_csv(bolo.data("fuse/per_levee_pd.csv"), index=False)
