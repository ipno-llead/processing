...
if __name__ == "__main__":
    cprr = pd.read_csv(dirk.data("clean/cprr_greenwood_pd_2015_2020.csv"))
    agency = cprr.agency[0]
    post = load_for_agency(agency)
    per = rearrange_personnel_columns(post)
    com = rearrange_allegation_columns(cprr)
    event = fuse_events(cprr, post)
    per.to_csv(dirk.data("fuse/per_greenwood_pd.csv"), index=False)
    com.to_csv(dirk.data("fuse/com_greenwood_pd.csv"), index=False)
    event.to_csv(dirk.data("fuse/event_greenwood_pd.csv"), index=False)
