...
if __name__ == "__main__":
    ...
    post_event = pd.read_csv(dirk.data("match/post_event_carencro_pd_2020_11_06.csv"))
    events_df = rearrange_event_columns(post_event)
    per_df = rearrange_personnel_columns(pprr)
    per_df.to_csv(dirk.data("fuse/per_carencro_pd.csv"), index=False)
    events_df.to_csv(dirk.data("fuse/event_carencro_pd.csv"), index=False)
