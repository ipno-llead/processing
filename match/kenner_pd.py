...
def extract_cprr_post_events(pprr, cprr_post):
    ...
    decision = 0.95
    matcher.save_pairs_to_excel(data_file_path(
        "match/pprr_kenner_pd_2020_v_cprr_post_2016_2019.csv.xlsx"), decision)
    matches = matcher.get_index_pairs_within_thresholds(lower_bound=decision)
...
if __name__ == '__main__':
    ...
    post_events.to_csv(
        data_file_path('match/post_event_kenner_pd_2020.csv'),
        index=False)
    uof.to_csv(
        data_file_path('match/uof_kenner_pd_2005_2021.csv'),
        index=False)
