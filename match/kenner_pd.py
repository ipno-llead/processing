...
def extract_post_events(pprr, post):
    ...
    matcher.save_pairs_to_excel(
        dirk.data("match/kenner_pd_pprr_2020_v_post_pprr_2020_11_06.xlsx"),
        decision,
    )
...
def match_uof_pprr(uof, ppprr):
    ...
    decision = 0.98
    matcher.save_pairs_to_excel(
        dirk.data("match/kenner_pd_uof_2005_2021_v_pprr_post_2020_11_06.xlsx"),
        decision,
    )
...
def extract_cprr_post_events(pprr, cprr_post):
    ...
    decision = 0.95
    matcher.save_pairs_to_excel(
        dirk.data("match/pprr_kenner_pd_2020_v_cprr_post_2016_2019.csv.xlsx"),
        decision,
    )
    matches = matcher.get_index_pairs_within_thresholds(lower_bound=decision)
...
if __name__ == "__main__":
    pprr = pd.read_csv(dirk.data("clean/pprr_kenner_pd_2020.csv"))
    agency = pprr.agency[0]
    post = load_for_agency(agency)
    uof = pd.read_csv(dirk.data("clean/uof_kenner_pd_2005_2021.csv"))
    ...
    post_events.to_csv(dirk.data("match/post_event_kenner_pd_2020.csv"), index=False)
    uof.to_csv(dirk.data("match/uof_kenner_pd_2005_2021.csv"), index=False)
    cprr_post_events.to_csv(
        dirk.data("match/cprr_post_events_kenner_pd_2020.csv"), index=False
    )
