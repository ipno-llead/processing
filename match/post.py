...
def match_cprr_with_nopd(cprr, pprr):
    ...
    matches = matcher.save_pairs_to_excel(data_file_path(
        'match/post_cprr_2016_2019_v_nopd_pprr.xlsx'), decision)
...
if __name__ == '__main__':
    pprr = pd.read_csv(data_file_path('clean/pprr_baton_rouge_so_2021.csv'))
    cprr = pd.read_csv(data_file_path('clean/cprr_post_2016_2019.csv'))
    cprr = match_cprr_with_nopd(cprr, pprr)
