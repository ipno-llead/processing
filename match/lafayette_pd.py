...
def dedup_cprr_uid_25(cprr):
    ...
    matcher.save_clusters_to_excel(
        deba.data("match/deduplicate_cprr_lafayette_pd_25.xlsx"),
        decision,
        decision,
    )
...
def dedup_cprr_investigator_uid_25(cprr):
    ...
    decision = 0.87
    matcher.save_clusters_to_excel(
        deba.data("match/lafayette_pd_cprr_2020_2025_investigator_dedup.xlsx"),
        decision,
    ...
    matcher.save_pairs_to_excel(
        deba.data(
            "match/lafayette_pd_cprr_2020_2025_investigator_only_last_name_dedup.xlsx"
        ),
        decision,
    )
...
def match_cprr_25_officers_with_pprr(cprr, pprr):
    dfa = (
...
def match_cprr_25_investigators_with_pprr(cprr, pprr):
    dfa = (
        cprr[["investigator_first_name", "investigator_last_name", "investigator_uid"]]
        .drop_duplicates()
        .set_index("investigator_uid", drop=True)
...
if __name__ == "__main__":
    ...
    cprr_25 = (
        dedup_cprr_uid_25(cprr_25)
        .pipe(dedup_cprr_investigator_uid_25)
        .pipe(match_cprr_25_officers_with_pprr, pprr)
        .pipe(match_cprr_25_investigators_with_pprr, pprr)
