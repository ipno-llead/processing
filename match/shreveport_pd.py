...
from datamatch import (
    ThresholdMatcher, JaroWinklerSimilarity, ColumnsIndex, StringSimilarity
)
...
def match_cprr_codebook(cprr, cb):
    ...
    matcher = ThresholdMatcher(ColumnsIndex('code'), {
        'name': StringSimilarity()
    }, dfa, cb)
    decision = 0.541
    lower_bound = 0.5
    matcher.save_pairs_to_excel(data_file_path(
        'match/shreveport_pd_cprr_2018_2019_v_codebook.xlsx'
...
if __name__ == '__main__':
    ...
    cb = pd.read_csv(data_file_path(
        'clean/cprr_codebook_shreveport_pd.csv'
    ))
    cprr = match_cprr_codebook(cprr, cb)\
