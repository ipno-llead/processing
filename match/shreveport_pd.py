...
def match_cprr21_post(cprr21, post):
    dfa = cprr21[["uid", "first_name", "last_name"]]
    dfa = dfa.drop_duplicates().set_index("uid")
    dfa.loc[:, "fc"] = dfa.first_name.fillna("").map(lambda x: x[:1])
    ...
    decision = 0.93
    matcher.save_pairs_to_excel(
        deba.data("match/shreveport_pd_cprr_2021_2023_v_post_pprr_2025_08_25.xlsx"),
        decision,
    )
    matches = matcher.get_index_pairs_within_thresholds(decision)
    match_dict = dict(matches)

    cprr21.loc[:, "uid"] = cprr21.uid.map(lambda x: match_dict.get(x, x))
    return cprr21    

if __name__ == "__main__":
    cprr = pd.read_csv(deba.data("clean/cprr_shreveport_pd_2018_2019.csv"))
    cprr21 = pd.read_csv(deba.data("clean/cprr_shreveport_pd_2021_2023.csv"))
    ...
    cprr21.to_csv(deba.data("match/cprr_shreveport_pd_2021_2023.csv"), index=False)
