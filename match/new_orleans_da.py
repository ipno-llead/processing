...
def match_cprr_and_post(cprr, post):
    ...
    matcher.save_pairs_to_excel(
        bolo.data("match/new_orleans_da_cprr_2021_v_post_nopd_pprr_2020_11_06.xlsx"),
        decision,
    )
...
def match_against_new_orleans_pd_personnel(cprr, pprr):
    ...
    decision = 1
    matcher.save_pairs_to_excel(
        bolo.data("match/new_orleans_da_cprr_2021_v_new_orleans_pd_1946_2018.xlsx"),
        decision,
    )
...
if __name__ == "__main__":
    pprr = pd.read_csv(bolo.data("clean/pprr_new_orleans_pd_1946_2018.csv"))
    cprr = pd.read_csv(bolo.data("clean/cprr_new_orleans_da_2021.csv"))
    agency = cprr.agency[0]
    post = load_for_agency(agency)
    cprr = match_against_new_orleans_pd_personnel(cprr, pprr)
    cprr = match_cprr_and_post(cprr, post)

    cprr.to_csv(bolo.data("match/cprr_new_orleans_da_2021.csv"), index=False)
