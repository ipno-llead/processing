...
def match_csd_and_pd_pprr(csd, pprr, year, decision):
    ...
    matcher.save_pairs_to_excel(
        dirk.data("match/baton_rouge_csd_pprr_%d_v_pd_pprr_2021.xlsx" % year),
        decision,
    )
...
def match_pd_cprr_2018_v_pprr(cprr, pprr):
    ...
    decision = 1
    matcher.save_pairs_to_excel(
        dirk.data("match/baton_rouge_pd_cprr_2018_v_pd_pprr_2021.xlsx"), decision
    )
    matches = matcher.get_index_pairs_within_thresholds(lower_bound=decision)
...
def match_pd_cprr_2021_v_pprr(cprr, pprr):
    ...
    decision = 1
    matcher.save_pairs_to_excel(
        dirk.data("match/baton_rouge_pd_cprr_2021_v_pd_pprr_2021.xlsx"), decision
    )
    matches = matcher.get_index_pairs_within_thresholds(lower_bound=decision)
...
def match_pprr_against_post(pprr, post):
    ...
    decision = 0.93
    matcher.save_pairs_to_excel(
        dirk.data("match/baton_rouge_pd_pprr_2021_v_post_pprr_2020_11_06.xlsx"),
        decision,
    )
    matches = matcher.get_index_pairs_within_thresholds(lower_bound=decision)
...
def match_lprr_against_pprr(lprr, pprr):
    ...
    decision = 1
    matcher.save_pairs_to_excel(
        dirk.data("match/baton_rouge_fpcsb_lprr_1992_2012_v_pprr_2021.xlsx"),
        decision,
    )
    matches = matcher.get_index_pairs_within_thresholds(lower_bound=decision)
    match_dict = dict(matches)
...
if __name__ == "__main__":
    ...
    pprr = pd.read_csv(dirk.data("clean/pprr_baton_rouge_pd_2021.csv"))
    ...
    lprr.to_csv(dirk.data("match/lprr_baton_rouge_fpcsb_1992_2012.csv"), index=False)
    csd17.to_csv(dirk.data("match/pprr_baton_rouge_csd_2017.csv"), index=False)
    csd19.to_csv(dirk.data("match/pprr_baton_rouge_csd_2019.csv"), index=False)
    cprr18.to_csv(dirk.data("match/cprr_baton_rouge_pd_2018.csv"), index=False)
    cprr21.to_csv(dirk.data("match/cprr_baton_rouge_pd_2021.csv"), index=False)
    post_event.to_csv(dirk.data("match/event_post_baton_rouge_pd.csv"), index=False)
