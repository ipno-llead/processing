...
def match_against_baton_rouge_csd_pprr(df, pprr, year, decision):
    ...
    matcher.save_pairs_to_excel(
        dirk.data("match/baton_rouge_da_cprr_2018_v_csd_pprr_%d.xlsx" % year),
        decision,
    )
...
def match_against_baton_rouge_so_personnel(df, per):
    ...
    decision = 1
    matcher.save_pairs_to_excel(
        dirk.data("match/baton_rouge_da_cprr_2018_v_baton_rouge_so_personnel.xlsx"),
        decision,
    )
    matches = matcher.get_index_pairs_within_thresholds(lower_bound=decision)
...
if __name__ == "__main__":
    pprr19 = pd.read_csv(dirk.data("match/pprr_baton_rouge_csd_2019.csv"))
    pprr17 = pd.read_csv(dirk.data("match/pprr_baton_rouge_csd_2017.csv"))
    # so_per = pd.read_csv(dirk.data("fuse/per_baton_rouge_so.csv"))
    df = pd.read_csv(dirk.data("clean/cprr_baton_rouge_da_2021.csv"))
    ...
    df.to_csv(dirk.data("match/cprr_baton_rouge_da_2021.csv"), index=False)
