...
def match_and_concat_pprr(df1, df2, year1, year2, decision):
    dfa = df1[['first_name', 'last_name', 'uid']]\
        .drop_duplicates(subset=['uid']).set_index('uid')
    dfa.loc[:, 'fc'] = dfa.first_name.map(lambda x: x[:1])

    dfb = df2[['first_name', 'last_name', 'uid']]\
        .drop_duplicates(subset=['uid']).set_index('uid')
    ...
    matcher.save_pairs_to_excel(data_file_path(
        "match/youngsville_pd_pprr_%s_v_%s.xlsx" % (year1, year2)), decision)
...
def match_pprr_and_post(pprr, post):
    ...
    decision = 0.9
    matcher.save_pairs_to_excel(data_file_path(
        "match/youngsville_pd_pprr_2017_2019_v_post_pprr_2020_11_06.xlsx"), decision)
    matches = matcher.get_index_pairs_within_thresholds(lower_bound=decision)
    return extract_events_from_post(post, matches, 'Youngsville PD')


if __name__ == '__main__':
    pprr17 = pd.read_csv(data_file_path(
        'clean/pprr_youngsville_csd_2017.csv'
    ))
    pprr18 = pd.read_csv(data_file_path(
        'clean/pprr_youngsville_csd_2018.csv'
    ))
    pprr19 = pd.read_csv(data_file_path(
        'clean/pprr_youngsville_csd_2019.csv'
    ))
    pprr = match_and_concat_pprr(
        match_and_concat_pprr(pprr17, pprr18, '2017', '2018', 0.9),
        pprr19, '2017_2018', '2019', 0.9
    )
