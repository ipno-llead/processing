...
def match_and_concat_pprr(df1, df2, year1, year2, decision):
    ...
    matcher.save_pairs_to_excel(
        data_file_path("match/youngsville_pd_pprr_%s_v_%s.xlsx" % (year1, year2)),
        decision,
    )
...
def match_pprr_and_post(pprr, post):
    ...
    matcher.save_pairs_to_excel(
        data_file_path(
            "match/youngsville_pd_pprr_2017_2019_v_post_pprr_2020_11_06.xlsx"
        ),
        decision,
    )
    matches = matcher.get_index_pairs_within_thresholds(lower_bound=decision)
...
if __name__ == "__main__":
    pprr17 = pd.read_csv(data_file_path("clean/pprr_youngsville_csd_2017.csv"))
    pprr18 = pd.read_csv(data_file_path("clean/pprr_youngsville_csd_2018.csv"))
    pprr19 = pd.read_csv(data_file_path("clean/pprr_youngsville_csd_2019.csv"))
