...
def match_csd_pprr_against_post_pprr(pprr, post):
    ...
    matcher.save_pairs_to_excel(
        dirk.data("match/port_allen_csd_pprr_2020_v_post_pprr_2020_11_06.xlsx"),
        decision,
    )
...
def match_cprr_against_csd_pprr_2020(cprr, pprr, year, decision):
    ...
    matcher.save_pairs_to_excel(
        dirk.data("match/port_allen_pd_cprr_%d_v_csd_pprr_2020.xlsx" % year),
        decision,
    )
    matches = matcher.get_index_pairs_within_thresholds(decision)
...
if __name__ == "__main__":
    pprr = pd.read_csv(dirk.data("clean/pprr_port_allen_csd_2020.csv"))
    cprr19 = pd.read_csv(dirk.data("clean/cprr_port_allen_pd_2019.csv"))
    cprr18 = pd.read_csv(dirk.data("clean/cprr_port_allen_pd_2017_2018.csv"))
    cprr16 = pd.read_csv(dirk.data("clean/cprr_port_allen_pd_2015_2016.csv"))
    ...
    pprr.to_csv(dirk.data("match/pprr_port_allen_csd_2020.csv"), index=False)
    cprr19.to_csv(dirk.data("match/cprr_port_allen_pd_2019.csv"), index=False)
    cprr18.to_csv(dirk.data("match/cprr_port_allen_pd_2017_2018.csv"), index=False)
    cprr16.to_csv(dirk.data("match/cprr_port_allen_pd_2015_2016.csv"), index=False)
    post_event.to_csv(dirk.data("match/post_event_port_allen_pd.csv"), index=False)
