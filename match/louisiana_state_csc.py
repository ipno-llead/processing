...
def match_cprr_19_to_pprr(cprr19, pprr):
    dfa = cprr19[["uid", "first_name", "last_name"]]
    dfa.loc[:, "fc"] = dfa.first_name.fillna("").map(lambda x: x[:1])
    dfa.loc[:, "lc"] = dfa.last_name.fillna("").map(lambda x: x[:1])
    dfa = dfa.drop_duplicates(subset=["uid"]).set_index("uid")
    ...
    matcher.save_pairs_to_excel(
        deba.data("match/cprr_lsp_2015_2019_v_pprr_lsp.xlsx"),
        decision,
    )
    matches = matcher.get_index_pairs_within_thresholds(lower_bound=decision)
    match_dict = dict(matches)

    cprr19.loc[:, "uid"] = cprr19.uid.map(lambda x: match_dict.get(x, x))
    return cprr19
...
if __name__ == "__main__":
    ...
    cprr19 = pd.read_csv(deba.data("clean/cprr_louisiana_state_pc_2015_2019.csv"))
    ...
    cprr19.to_csv(deba.data("match/cprr_louisiana_state_pc_2015_2019.csv"), index=False)
