...
def match_lprr_and_pprr(lprr, pprr):
    ...
    matcher.save_pairs_to_excel(
        dirk.data("match/louisiana_state_csc_lprr_1991_2020_v_csd_pprr_2021.xlsx"),
        decision,
    )
...
def extract_post_events(pprr, post):
    ...
    decision = 0.924
    matcher.save_pairs_to_excel(
        dirk.data("match/louisiana_state_csd_pprr_2021_v_post_pprr_2020_11_06.xlsx"),
        decision,
    )
    matches = matcher.get_index_pairs_within_thresholds(lower_bound=decision)
...
if __name__ == "__main__":
    lprr = pd.read_csv(dirk.data("clean/lprr_louisiana_state_csc_1991_2020.csv"))
    pprr_demo = pd.read_csv(dirk.data("clean/pprr_demo_louisiana_csd_2021.csv"))
    ...
    lprr.to_csv(dirk.data("match/lprr_louisiana_state_csc_1991_2020.csv"), index=False)
    post_events.to_csv(
        dirk.data("match/post_event_louisiana_state_police_2020.csv"), index=False
    )
    pprr_term.to_csv(dirk.data("match/pprr_term_louisiana_csd_2021.csv"), index=False)
