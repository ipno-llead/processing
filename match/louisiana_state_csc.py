...
from datamatch import (
    ThresholdMatcher, JaroWinklerSimilarity, ColumnsIndex, Swap, NonOverlappingFilter
)
import pandas as pd
...
def match_pprr_demo_and_term(demo, term):
    ...
    dfa.loc[:, 'end_date'] = combine_date_columns(demo, 'hire_year', 'hire_month', 'hire_day')
    dfa.loc[:, 'fc'] = dfa.first_name.fillna('').map(lambda x: x[:1])
    dfa = dfa.sort_values(['uid', 'end_date'], na_position='first')\
        .drop_duplicates(subset=['uid'], keep='last')\
        .set_index('uid', drop=True)

    dfb = term[['uid', 'first_name', 'last_name']]
    dfb.loc[:, 'start_date'] = combine_date_columns(term, 'left_year', 'left_month', 'left_day')
    dfb.loc[:, 'end_date'] = datetime(2100, 1, 1)
    dfb.loc[:, 'fc'] = dfb.first_name.fillna('').map(lambda x: x[:1])
    dfb = dfb.sort_values(['uid', 'start_date'], na_position='last')\
    ...
    matcher = ThresholdMatcher(ColumnsIndex(['fc']), {
        'first_name': JaroWinklerSimilarity(),
        'last_name': JaroWinklerSimilarity(),
    }, dfa, dfb, filters=[NonOverlappingFilter('start_date', 'end_date')])
    decision = 0.98
    matcher.save_pairs_to_excel(data_file_path(
        "match/louisiana_state_csd_pprr_demographic_2021_v_terminations_2021.xlsx"), decision)
    matches = matcher.get_index_pairs_within_thresholds(lower_bound=decision)
    match_dict = dict(matches)
...
if __name__ == '__main__':
    ...
    lprr.to_csv(
        data_file_path('match/lprr_louisiana_state_csc_1991_2020.csv'),
        index=False)
    post_events.to_csv(
        data_file_path("match/post_event_louisiana_state_police_2020.csv"),
        index=False)
    pprr_term.to_csv(
        data_file_path('match/pprr_term_louisiana_csd_2021.csv'),
        index=False)
