...
def match_cprr_and_pprr(cprr, pprr):
    ...
    matcher.save_pairs_to_excel(
        deba.data("match/madisonville_pd_cprr_2010_2020_v_csd_pprr_2019.xlsx"),
        decision,
    )
...
def match_pprr_and_post(pprr, post):
    ...
    decision = 0.95
    matcher.save_pairs_to_excel(
        deba.data("match/madisonville_csd_pprr_2019_v_post_pprr_2020_11_06.xlsx"),
        decision,
    )
    matches = matcher.get_index_pairs_within_thresholds(lower_bound=decision)
...
if __name__ == "__main__":
    cprr = pd.read_csv(deba.data("clean/cprr_madisonville_pd_2010_2020.csv"))
    ...
    cprr.to_csv(deba.data("match/cprr_madisonville_pd_2010_2020.csv"), index=False)
    post_event.to_csv(
        deba.data("match/post_event_madisonville_csd_2019.csv"), index=False
    )
