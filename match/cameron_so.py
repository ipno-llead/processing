...
def assign_uid_from_post_20(cprr, post):
    ...
    matcher.save_pairs_to_excel(
        deba.data("match/cprr_cameron_so_2020_v_post_pprr_2020_11_06.xlsx"),
        decision,
    )
...
def assign_uid_from_post_19(cprr, post):
    ...
    decision = 1
    matcher.save_pairs_to_excel(
        deba.data("match/cprr_cameron_so_2015_2019_v_post_pprr_2020_11_06.xlsx"),
        decision,
    )
    matches = matcher.get_index_pairs_within_thresholds(decision)
    match_dict = dict(matches)
...
def assign_uid_from_post_14(cprr, post):
    dfa = (
        cprr.loc[cprr.uid.notna(), ["uid", "first_name", "last_name"]]
        .drop_duplicates(subset=["uid"])
        .set_index("uid", drop=True)
    ...
    decision = 1
    matcher.save_pairs_to_excel(
        deba.data("match/cprr_cameron_so_2013_2014_v_post_pprr_2020_11_06.xlsx"),
        decision,
    )
    matches = matcher.get_index_pairs_within_thresholds(decision)
    match_dict = dict(matches)
...
if __name__ == "__main__":
    cprr_20 = pd.read_csv(deba.data("clean/cprr_cameron_so_2020.csv"))
    cprr_19 = pd.read_csv(deba.data("clean/cprr_cameron_so_2015_2019.csv"))
    ...
    cprr_20.to_csv(deba.data("match/cprr_cameron_so_2020.csv"), index=False)
    cprr_19.to_csv(deba.data("match/cprr_cameron_so_2015_2019.csv"), index=False)
    cprr_14.to_csv(deba.data("match/cprr_cameron_so_2013_2014.csv"), index=False)
