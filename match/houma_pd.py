...
def match_cprr_18_with_post(cprr, post):
    ...
    decision = .87
    matcher.save_pairs_to_excel(
        data_file_path('match/cprr_houma_pd_2017_2019_v_post_pprr_2020_11_06.xlsx'), decision)
    matches = matcher.get_index_pairs_within_thresholds(decision)
    match_dict = dict(matches)
...
if __name__ == '__main__':
    ...
    cprr21.to_csv(data_file_path(
        'match/cprr_houma_pd_2019_2021.csv'), index=False)
    cprr18.to_csv(data_file_path(
        'match/cprr_houma_pd_2017_2018.csv'), index=False)
