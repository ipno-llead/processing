...
def match_cprr_18_against_post(cprr, post):
    ...
    matcher.save_pairs_to_excel(
        data_file_path("match/baton_rouge_so_cprr_2018_v_post_pprr_2020_11_06.xlsx"),
        decision,
    )
...
def match_cprr_20_against_post(cprr, post):
    ...
    decision = 1
    matcher.save_pairs_to_excel(
        data_file_path("match/baton_rouge_so_cprr_2020_v_post_pprr_2020_11_06.xlsx"),
        decision,
    )
    matches = matcher.get_index_pairs_within_thresholds(lower_bound=decision)
    match_dict = dict(matches)
...
if __name__ == "__main__":
    ...
    cprr18.to_csv(data_file_path("match/cprr_baton_rouge_so_2018.csv"), index=False)
    cprr20.to_csv(data_file_path("match/cprr_baton_rouge_so_2016_2020.csv"), index=False)
