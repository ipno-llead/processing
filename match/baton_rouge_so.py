...
def match_settlements_against_post(settlement, post):
    dfa = settlement[["uid", "first_name", "last_name"]]
    dfa.loc[:, "fc"] = dfa.first_name.fillna("").map(lambda x: x[:1])
    dfa = dfa.drop_duplicates(subset=["uid"]).set_index("uid")
    ...
    matches = matcher.get_index_pairs_within_thresholds(lower_bound=decision)
    match_dict = dict(matches)
    settlement.loc[:, "uid"] = settlement.uid.map(lambda x: match_dict.get(x, x))
    return settlement
...
if __name__ == "__main__":
    ...
    settlements = match_settlements_against_post(settlements, post)
    ...
    settlements.to_csv(deba.data("match/settlements_baton_rouge_so_2021_2023.csv"), index=False)
