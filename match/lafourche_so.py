...
def match_cprr22_post(cprr, post):
    dfa = cprr[["uid", "first_name", "last_name"]]
    dfa = dfa.drop_duplicates(subset=["uid"]).set_index("uid")
    dfa.loc[:, "fc"] = dfa.first_name.fillna("").map(lambda x: x[:1])
    ...
    decision = 0.891
    matcher.save_pairs_to_excel(
        deba.data("match/cprr_lafourche_so_2022_2023_v_pprr_post_2020_11_06.xlsx"),
        decision,
    )
    matches = matcher.get_index_pairs_within_thresholds(decision)
    match_dict = dict(matches)
...
if __name__ == "__main__":
    ...
    cprr22 = pd.read_csv(deba.data("clean/cprr_lafourche_so_2022_2023.csv"))
    ...
    cprr22 = match_cprr22_post(cprr22, post)
    ...
    cprr22.to_csv(deba.data("match/cprr_lafourche_so_2022_2023.csv"), index=False)
