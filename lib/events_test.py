...
from events import (
    ...
    OFFICER_PAY_EFFECTIVE, OFFICER_RANK
)
import sys
sys.path.append('./')
...
class EventsBuilderTestCase(unittest.TestCase):
    def assert_events_frame_equal(self, builder, data, columns):
        ...
        assert_frame_equal(builder.to_frame(), df)
    ...
    def test_extract_events(self):
        ...
        df3 = pd.DataFrame([
            ['1236', 'Brusly PD', '20200720', '43210.98',
                'police officer', '20200311 1000'],
            ['1236', 'Brusly PD', '20210112',
                '54321.76', 'sergeant', '20200311 1000'],
        ], columns=['uid', 'agency', 'effective_date', 'annual_salary', 'rank_desc', 'hire_datetime'])
        builder.extract_events(df3, {
            OFFICER_HIRE: {
                'prefix': 'hire', 'parse_datetime': '%Y%m%d %H%M', 'drop': ['rank_desc', 'annual_salary']},
            OFFICER_PAY_EFFECTIVE: {
                'prefix': 'effective', 'parse_date': '%Y%m%d', 'drop': ['rank_desc']},
            OFFICER_RANK: {
                'prefix': 'effective', 'parse_date': '%Y%m%d', 'drop': ['annual_salary'],
                'id_cols': ['uid', 'rank_desc']},
        }, ['uid'])

        self.assert_events_frame_equal(builder, [
            ['ad25f5c06789fa96291a9b9c384a4e52', OFFICER_HIRE,
                '2020', '5', '3', np.NaN, np.NaN, '1234', np.NaN, 'Brusly PD', np.NaN, np.NaN],
            ['1f79d27b602f0d8b9751f69ee2abf865', OFFICER_LEFT,
                '2021', '2', '6', np.NaN, np.NaN, '1234', np.NaN, 'Brusly PD', np.NaN, np.NaN],
            ['cb0c3e5a857bc0c96706777faf5c44fb', OFFICER_HIRE,
                '2019', '10', '12', np.NaN, np.NaN, '1235', np.NaN, 'Brusly PD', np.NaN, np.NaN],
            ['e6ffcd655260bd8f9914bd4aec566cec', COMPLAINT_INCIDENT,
                '2020', '6', '15', '12:30', '6/15/2020 12:30', '1234', 'C1122', 'Brusly PD', np.NaN, np.NaN],
            ['f1d6ade08cff774817e2ffba94badb4c', COMPLAINT_RECEIVE,
                '2020', '6', '18', np.NaN, '6/18/2020', '1234', 'C1122', 'Brusly PD', np.NaN, np.NaN],
            ['ec6db5f177e2077d5bb278c9c9b4b0bb', COMPLAINT_INCIDENT,
                '2020', '3', '20', '10:00', '3/20/2020 10:00', '1235', 'C2233', 'Brusly PD', np.NaN, np.NaN],
            ['781bb698a6cb3fde10bc904d54353b31', COMPLAINT_RECEIVE,
                '2020', '3', '28', np.NaN, '3/28/2020', '1235', 'C2233', 'Brusly PD', np.NaN, np.NaN],
            ['c948a2e1571d10af2b46f9de33c3354f', OFFICER_HIRE, '2020', '3', '11',
                '10:00', '20200311 1000', '1236', np.NaN, 'Brusly PD', np.NaN, np.NaN],
            ['b7a8d5dba5195dcc4a395fba74c8bbec', OFFICER_PAY_EFFECTIVE, '2020', '7',
                '20', np.NaN, '20200720', '1236', np.NaN, 'Brusly PD', np.NaN, '43210.98'],
            ['bf4c14ab4059054b56a2c3bdc07ffbc8', OFFICER_RANK, '2020', '7', '20', np.NaN,
                '20200720', '1236', np.NaN, 'Brusly PD', 'police officer', np.NaN],
            ['21c2d069f6276c756534b89b80d2f653', OFFICER_PAY_EFFECTIVE, '2021', '1',
                '12', np.NaN, '20210112', '1236', np.NaN, 'Brusly PD', np.NaN, '54321.76'],
            ['0ce57935f21186e98875e0b12d479545', OFFICER_RANK, '2021', '1', '12',
                np.NaN, '20210112', '1236', np.NaN, 'Brusly PD', 'sergeant', np.NaN],
        ], ['event_uid', 'kind', 'year', 'month', 'day', 'time', 'raw_date',
            'uid', 'complaint_uid', 'agency', 'rank_desc', 'annual_salary'])
